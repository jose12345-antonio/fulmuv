...
let traccion = [];
let motor = [];
let tipo_user = "empresa"; // admin siempre opera a nivel empresa
...
function firstSimpleValue(value, idKeys = []) {
  const arr = safeArr(value);
  if (!arr.length) return '';
  const first = arr[0];
  if (first && typeof first === 'object') {
    for (const key of idKeys) {
      if (first[key] != null && first[key] !== '') return String(first[key]);
    }
    if (first.id != null && first.id !== '') return String(first.id);
...
function labelFromCatalogArray(value, id, idKeys = []) {
  const idStr = String(id || '').trim();
  const found = safeArr(value).find(item => {
    if (!item || typeof item !== 'object') return false;
    const itemIds = [...idKeys, 'id', 'value']
      .map(key => item[key])
      .filter(v => v != null && v !== '')
      .map(v => String(v));
    return itemIds.includes(idStr);
  });
  if (!found) return '';
  return String(found.nombre || found.text || found.label || idStr);
}

function ensureSelectOption($select, value, label = '') {
  const val = String(value || '').trim();
  if (!val) return false;
  const selectorValue = val.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  if (!$select.find(`option[value="${selectorValue}"]`).length) {
    $select.append(new Option(label || val, val));
  }
  $select.val(val).trigger('change');
  return true;
}

function aplicarSeleccionesRegistradasVehiculo() {
  if (!vehiculoEditData) return;
  const tipoId = firstSimpleValue(vehiculoEditData.tipo_auto, ['id_tipo_auto']);
  const traccionId = firstSimpleValue(vehiculoEditData.tipo_traccion, ['id_tipo_traccion']);
  const motorId = firstSimpleValue(vehiculoEditData.funcionamiento_motor, ['id_funcionamiento_motor']);

  if (tipoId) {
    ensureSelectOption(
      $("#tipo_vehiculo"),
      tipoId,
      labelFromCatalogArray(vehiculoEditData.tipo_autoArray, tipoId, ['id_tipo_auto'])
    );
  }
  if (traccionId) {
    ensureSelectOption(
      $("#traccion"),
      traccionId,
      labelFromCatalogArray(vehiculoEditData.tipo_traccionArray, traccionId, ['id_tipo_traccion'])
    );
  }
  if (motorId) {
    ensureSelectOption(
      $("#motor"),
      motorId,
      labelFromCatalogArray(vehiculoEditData.funcionamiento_motorArray, motorId, ['id_funcionamiento_motor'])
    );
  }
}
...
$(document).ready(function () {
  ...
  $.get('../api/v1/fulmuv/getReferencias/', {}, function (r) {
    ...
    if (!d.error) {
      ...
      if (vehiculoEditData) {
        ...
        aplicarSeleccionesRegistradasVehiculo();
  ...
  $.get('../api/v1/fulmuv/tiposAuto/', {}, function (r) {
    ...
    if (!d.error) {
      ...
      if (vehiculoEditData) aplicarSeleccionesRegistradasVehiculo();
    }
  });
...
function asignarModelo(){
  ...
  const esModeloRegistrado = vehiculoEditData && String(id_modelo) === String(vehiculoEditData.id_modelo || '');
