...
function cargarGaleria(callback) {
  $.get('../api/v1/fulmuv/filesEmpresa/' + getEmpresaId(), {}, function (returnedData) {
...
$(document).ready(function () {
  // Carga inicial cuando la empresa ya está seleccionada
  if (getEmpresaId()) cargarGaleria();
});

// Recargar galería cuando cambia la empresa en el selector global
...
function updateGaleria(id_archivo_empresa){
  ...
  $.post('../api/v1/fulmuv/updateFileEmpresa', {
    ...
    const returned = typeof returnedData === 'string' ? JSON.parse(returnedData) : returnedData;
    if (returned.error === false) {
      SweetAlert("url_success", returned.msg, "galeria.php?id_empresa="+getEmpresaId());
    } else {
...
function editImagen(id_archivo_empresa){
  $.get('../api/v1/fulmuv/fileEmpresaById/' + id_archivo_empresa, {}, function (returnedData) {
              ...
              <div class="p-4">
                ...
                <div class="row g-3">
                  ...
                  <div class="col-md-5">
                    ...
                    <div class="gallery-preview-meta">Si seleccionas una nueva imagen, se reemplazará la actual.</div>
                    <div class="gallery-upload-box mt-3">
                      <label class="form-label mb-2" for="edit_archivo">Nueva imagen opcional</label>
                      <input class="form-control" id="edit_archivo" type="file" accept="image/*"/>
                      <div class="gallery-preview-meta">Déjalo vacío si solo quieres actualizar título o descripción.</div>
                    </div>
                  </div>
    ...
    $(document).off('change', '#edit_archivo').on('change', '#edit_archivo', function(){
      const file = this.files && this.files[0];
      if (!file) {
        $('#edit_preview').attr('src', src);
        return;
      }

      const objectUrl = URL.createObjectURL(file);
      $('#edit_preview').attr('src', objectUrl);
    });
    ...
    $(document).off('click', '#btnUpdateGaleria').on('click', '#btnUpdateGaleria', function(){
      ...
      const selectedFile = $('#edit_archivo')[0]?.files?.[0];
      const filePromise = selectedFile ? saveFiles(selectedFile) : Promise.resolve(null);

      filePromise.then(function (file) {
        $.post('../api/v1/fulmuv/updateFileEmpresa', {
          id_archivo_empresa: id_archivo_empresa,
          id_empresa: getEmpresaId(),
          titulo: tituloN,
          descripcion: descN,
          archivo: file?.img || ''
    ...
    $.post('../api/v1/fulmuv/deleteFileEmpresa', {
      ...
      if (returned.error === false) {
        ...
        if ($btn.length) {
          ...
          // fallback: recargar
          SweetAlert("url_success", returned.msg || "Archivo eliminado", "galeria.php?id_empresa=" + getEmpresaId());
        }
      } else {
...
function addImagen() {
          ...
          <div class="modal-body p-0">
            ...
            <div class="p-4">
              ...
              <div class="row g-3">
                ...
                <div class="col-md-5">
                  <label class="form-label" for="archivo">Imagen a registrar</label>
                  <div class="ratio ratio-16x9 gallery-preview-box mb-3">
                    <img id="create_preview" src="" alt="preview" style="object-fit:cover;width:100%;height:100%;display:none;">
                    <div id="create_preview_empty" class="d-flex align-items-center justify-content-center h-100 text-600 small text-center p-3">
                      Selecciona una imagen para ver una previsualización aquí.
                    </div>
                  </div>
  ...
  $(document).off('change', '#archivo').on('change', '#archivo', function(){
    const file = this.files && this.files[0];
    if (!file) {
      $('#create_preview').hide().attr('src', '');
      $('#create_preview_empty').show();
      return;
    }

    const objectUrl = URL.createObjectURL(file);
    $('#create_preview').attr('src', objectUrl).show();
    $('#create_preview_empty').hide();
  });
