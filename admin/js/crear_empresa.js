  ...
  let responseCallback = response => {
    ...
    if (response.card) {
      ...
      if (String(response.card.status || '').toLowerCase() == "valid" && usableToken) {
        ...
        completarRegistroEmpresaConCobro(usableToken, transactionReference, cardMeta).then(function () {
      ...
      if (response.error.type.includes("Card already added")) {
        ...
        completarRegistroEmpresaConCobro(usableToken, transactionReference || null, cardMeta).then(function () {
          submitButton.style.display = 'none';
          $("#modal-pago").modal('hide');
          resetCheckoutState({ keepDraftHandled: true });
          swal({
