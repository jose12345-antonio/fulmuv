...
$(function () {
    ...
    function goStep2() {
        const $panel2 = $('#step2_panel'); // Asegúrate que el ID sea correcto
        if ($panel2.length) {
            $('#step1_panel').addClass('d-none');
            $panel2.removeClass('d-none');
            
            // Animación suave corregida
            window.scrollTo({
                top: $panel2.offset().top - 80,
                behavior: 'smooth'
            });
        }
    }
    function goStep1() {
        const $panel1 = $('#step1_panel');
        if ($panel1.length) {
            $('#step2_panel').addClass('d-none');
            $panel1.removeClass('d-none');
            
            window.scrollTo({
                top: $panel1.offset().top - 80,
                behavior: 'smooth'
            });
        }
    }
...
function initMap() {
    ...
    map = new google.maps.Map(document.getElementById("mapaEntrega"), {
        center: defaultPos,
        zoom: 14,
        // ✅ Configuraciones para que se vea como en la imagen que subiste
        mapTypeControl: true,
    ...
    searchBox.addListener('places_changed', () => {
        ...
        map.setZoom(17);
        // marker es tu variable global definida en shop-checkout.js
        marker.setPosition(place.geometry.location); 
        
        $("#direccion_mapa").val(place.formatted_address);
    });
}
...
function actualizarshopCart() {
    ...
    carrito.forEach(item => {
        ...
        $("#listaTotalProductosCheckOut").append(` 
            <tr>
                <td class="image product-thumbnail summary-product-thumb">
                    <img src="../${item.imagen}" style="width:60px;height:60px;object-fit:cover" onerror="this.src='../img/FULMUV-NEGRO.png';">
                </td>
                <td class="text-start">
                    <h6 class="summary-product-name mb-0"><a href="#" class="text-heading">${item.nombre}</a></h6> 
                </td>
                <td class="summary-product-qty"><h6 class="text-muted mb-0">x ${item.cantidad}</h6></td>
                <td class="summary-money-cell">${htmlPrecio}</td>
            </tr>
        `);
    });
    ...
    <tr class="summary-total-row">
      <td colspan="3"><strong class="summary-total-label">Subtotal</strong></td>
      <td class="summary-total-value"><strong>${formatoUSD(totalValorNeto)}</strong></td>
    </tr>
    <tr class="summary-total-row">
      <td colspan="3"><strong class="summary-total-label text-danger">Ahorro por Descuento</strong></td>
      <td class="summary-total-value"><strong class="text-danger">-${formatoUSD(totalAhorroCupon)}</strong></td>
    </tr>
    <tr class="summary-total-row">
      <td colspan="3"><strong class="summary-total-label">IVA (15%)</strong></td>
      <td class="summary-total-value"><strong>${formatoUSD(totalIVAGlobal)}</strong></td>
    </tr>
    <tr class="summary-total-row total">
      <td colspan="3"><strong class="summary-total-label">Total a Pagar</strong></td>
      <td class="summary-total-value"><strong>${formatoUSD(totalFinal)}</strong></td>
    </tr>
  `);
...
//     if (validateStep1()) {
//         $("#step1_panel").addClass("d-none");
//         $("#step2_panel").removeClass("d-none");
//         $("#m-step-1").css("opacity", "0.4").find(".num").html("✓");
//         $("#m-step-2").css("opacity", "1").addClass("active");
//         window.scrollTo(0, 0);
//     } 
// });
...
//     $("#step2_panel").addClass("d-none");
//     $("#step1_panel").removeClass("d-none");
//     $("#m-step-2").removeClass("active").css("opacity", "0.4");
//     $("#m-step-1").css("opacity", "1").find(".num").html("1");
// });
