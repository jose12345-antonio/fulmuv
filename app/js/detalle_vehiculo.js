...
function registrarInteraccionVehiculoWhatsapp(origen) {
    registrarInteraccionVehiculo("click_whatsapp", { metadata: { origen: origen || "detalle_vehiculo_app" } });
}

function registrarInteraccionVehiculoTelefono(origen) {
    registrarInteraccionVehiculo("click_telefono", { metadata: { origen: origen || "detalle_vehiculo_app" } });
}
...
$(document).ready(function () {
    ...
    $.post("../api/v1/fulmuv/vehiculos/byIdVehiculo", { id_vehiculo: id_producto }, function (returnedData) {
        ...
        if (!returnedData.error) {
            ...
            $("#viewVisitaTienda").html(`
                ...
                <a href="productos_vendor.php?q=${returnedData.data.empresa.id_empresa}"
                    ...
                    <span>${returnedData.data.empresa.nombre}</span>
                </a>
                `);
            ...
            const telMasked = obtenerTelefonoEnmascarado(telRaw);
            ...
            $("#contactosEmpresa").html(`
                <div class="contact-line">
                    ...
                    <span>Teléfono:</span>
                    ${telDigits ? `
                        <span class="text-muted">${telMasked}</span>
                        <a href="#" class="link-telefono-reveal">Ver más</a>
                        <a href="${telLink}" class="link-telefono-empresa d-none">${prettyPhone(telRaw)}</a>
                ...
                <div class="contact-line">
                    ...
                    <span>WhatsApp:</span>
                    ${waDigits ? `<a href="${waChat}" target="_blank" rel="noopener" class="link-whatsapp-empresa">${prettyPhone(waRaw)}</a>` : `<span class="text-muted">No disponible</span>`}
                </div>
                `);
            ...
            $('#btnWhatsApp').attr({
                ...
                e.preventDefault();
                registrarInteraccionVehiculoWhatsapp("detalle_vehiculo_app_btn");
                abrirWhatsAppApp(phone, waText, waApi);
            });

            $('.link-whatsapp-empresa').off('click').on('click', function (e) {
                e.preventDefault();
                registrarInteraccionVehiculoWhatsapp("detalle_vehiculo_app_contacto");
                abrirWhatsAppApp(phone, waText, waApi);
            });

            $('.link-telefono-reveal').off('click').on('click', function (e) {
                e.preventDefault();
                registrarInteraccionVehiculoTelefono("detalle_vehiculo_app_reveal");
                $('.link-telefono-empresa').removeClass('d-none');
                $(this).remove();
            });

            $('.link-telefono-empresa').off('click').on('click', function (e) {
                e.preventDefault();
                registrarInteraccionVehiculoTelefono("detalle_vehiculo_app_call");
                abrirLlamadaApp(telRaw || telDigits);
            });
        ...
        `);
            ...
            $('.detail-gallery .zoom-icon').off('click.detalleZoom').on('click.detalleZoom', function (e) {
                e.preventDefault();
                const currentSlideLink = $('.product-image-slider .slick-current a[data-fancybox="galeria-producto"]').first();
                if (currentSlideLink.length) {
                    currentSlideLink[0].click();
                }
            });
                    ...
                    <div class="product-cart-wrap">
                        <div class="product-img-action-wrap text-center">
                            <div class="product-img product-img-zoom">
                                <a href="detalle_vehiculo.php?q=${vehiculo.id_vehiculo}">
