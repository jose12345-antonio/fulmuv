...
function updateFilterActiveCount() {
    ...
    const precioCount   = (precioMin > 0 || precioMax !== Infinity ? 1 : 0);
    const marcaCount    = marcaVehSeleccionadas.size;
    const modeloCount   = modelosSeleccionados.size;
    const anioCount     = (anioMinSel !== null ? 1 : 0) + (anioMaxSel !== null ? 1 : 0);
    const condicionCount= condicionSeleccionadas.size;
    const tipoAutoCount = tipoAutoSeleccionados.size;
    const colorCount    = colorSeleccionados.size;
    const tapiceriaCount= tapiceriaSeleccionados.size;
    const climaCount    = climatizacionSeleccionados.size;
    const referenciaCount = referenciasSeleccionadas.size;

    const totalSpecs = precioCount + marcaCount + modeloCount + anioCount + condicionCount + tipoAutoCount + colorCount + tapiceriaCount + climaCount + referenciaCount;

    $("#filterActiveCount").text(locationCount + totalSpecs + (searchText ? 1 : 0));
    $("#filterGroupCountLocation").text(locationCount);
    $("#filterGroupCountPrecio").text(precioCount);
    $("#filterGroupCountMarca").text(marcaCount);
    $("#filterGroupCountModelo").text(modeloCount);
    $("#filterGroupCountAnio").text(anioCount);
    $("#filterGroupCountCondicion").text(condicionCount);
    $("#filterGroupCountTipoAuto").text(tipoAutoCount);
    $("#filterGroupCountColor").text(colorCount);
    $("#filterGroupCountTapiceria").text(tapiceriaCount);
    $("#filterGroupCountClimatizacion").text(climaCount);
    $("#filterGroupCountReferencia").text(referenciaCount);
}
...
$(document).on("change", "input[name='checkbox']", function () {
    if (this.value === "__all__") {
        ...
        $.post("../api/v1/fulmuv/productos/idCategoria", { id_categoria: idsTodas }, function (rd) {
            if (!rd.error) {
                ...
                subcategoriasHijas = []; subcategoriasHijasSeleccionadas = []; $("#filtro-sub-categorias").html(""); $("#subcats-box").hide(); updateFilterActiveCount(); renderEmpresas(productosData, 1);
            }
        }, 'json');
        return;
    }
    ...
    $("#cat-all").prop("checked", subcategoriasSeleccionadas.length === 0);
...
function paintCheckboxes(containerSel, mapOrSet, group) {
    ...
    </div>`; const entries = mapOrSet instanceof Map ? Array.from(mapOrSet.entries()) : Object.entries(mapOrSet || {});
    entries.sort((a, b) => a[1].localeCompare(b[1])).forEach(([val, label]) => {
        const id = `${group}-${cssSafe(val)}`;
        const checked = selectedSet.has(String(val)) || selectedSet.has(val);
        html += `<div class="form-check mb-2">
      <input class="form-check-input chk-${group}" type="checkbox" id="${id}" value="${val}" ${checked ? 'checked' : ''}>
...
function getSelectedSetForVehicleGroup(group) {
    switch (group) {
        case "modelo": return modelosSeleccionados;
        case "marcav": return marcaVehSeleccionadas;
        case "condicion": return condicionSeleccionadas;
        case "tipoauto": return tipoAutoSeleccionados;
        case "color": return colorSeleccionados;
        case "tapiceria": return tapiceriaSeleccionados;
        case "clima": return climatizacionSeleccionados;
        case "referencia": return referenciasSeleccionadas;
        default: return new Set();
    }
}

function bindVehicleCheckboxGroup(selector, selectedSet, rerenderFilters = false) {
    $(document).off("change", selector).on("change", selector, function () {
        ...
        if (v === "__all__") {
            ...
            this.checked ? selectedSet.add(v) : selectedSet.delete(v);
            $(`${selector}[value="__all__"]`).prop("checked", selectedSet.size === 0);
        }

        if (rerenderFilters) refreshDependentFilters();
        updateFilterActiveCount();
        renderEmpresas(productosData, 1);
    });
}
...
function buildVehicleFilters(dataset) {
    ...
    paintCheckboxes("#filtro-referencias", toMap(referencias), "referencia");
...
function bindVehicleFiltersUI() {
    bindVehicleCheckboxGroup(".chk-modelo", modelosSeleccionados, true);
    bindVehicleCheckboxGroup(".chk-marcav", marcaVehSeleccionadas, true);
    bindVehicleCheckboxGroup(".chk-condicion", condicionSeleccionadas, false);
    bindVehicleCheckboxGroup(".chk-tipoauto", tipoAutoSeleccionados, false);
    bindVehicleCheckboxGroup(".chk-color", colorSeleccionados, false);
    bindVehicleCheckboxGroup(".chk-tapiceria", tapiceriaSeleccionados, false);
    bindVehicleCheckboxGroup(".chk-clima", climatizacionSeleccionados, false);
    bindVehicleCheckboxGroup(".chk-referencia", referenciasSeleccionadas, false);
...
function renderEmpresas(data, page = 1) {
    ...
    let empresasFiltradas = data.filter(prod => {
        ...
        return matchSearch && matchPrimeraCatProducto && matchCat && matchSubHija
            ...
            && matchColor && matchTapiceria && matchClima && matchReferencia
