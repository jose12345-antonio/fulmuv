...
function updateFilterActiveCount() {
    const count =
        ...
        (provinciaSel.id ? 1 : 0) +
        (cantonSel.id ? 1 : 0) +
        ((precioMin > 0 || (Number.isFinite(precioMax) && allPriceMax > 0 && precioMax < allPriceMax)) ? 1 : 0);
...
$(document).ready(function () {
    ...
    $.post("../api/v1/fulmuv/productos/idEmpresa", { id_empresa }, function (res) {
        if (!res.error) {
            productosData = Array.isArray(res.data) ? res.data : [];
...
function renderProductos(data, page = 1) {
    ...
    let filtrados = source.filter(prod => {
        ...
        return matchSearch && matchCat && matchSub && matchMarca && matchModelo && matchPrecio;
    });
    ...
    paginados.forEach(prod => {
        ...
        html += `
            <article class="product-card-modern">
                <a onclick="irADetalleProductoConTerminos(${prod.id_producto})" class="product-card-link">
    ...
    $("#listaProductosContainer").html(html || buildEmptyStateHtml(
        ...
        "Prueba con otra marca, modelo, categoria, subcategoria o rango de precio para ver resultados disponibles en esta tienda."
    ));
...
function refreshFiltersForCurrentLocation() {
    ...
    inicializarSlider(getMaxPrice(locData));
}
...
function getDetalleUrl(prod) {
    ...
    return `${detailPath}?q=${prod.id_producto}`;
}

function getMarcaNombres(prod) {
    return toArray(prod.marca).map(item => capitalizarPrimeraLetra(item?.nombre)).filter(Boolean);
}
