...
function scrollToResultsTop() {
    ...
    const resultsContainer = resultsHead || document.getElementById("listaProductosContainer");
...
$(document).ready(function () {
    ...
    $("#clearFiltersPanel").on("click", clearAllFilters);
    $("#applyFiltersPanel").on("click", function () {
        closeFilterPanel();
  ...
  `);
    ...
    $.get("../api/v1/fulmuv/categoriasByPrincipales/" + id_categoria, function (returnedDataCategoria) {
        ...
        $.post("../api/v1/fulmuv/productos/idCategoria", { id_categoria: idsCategorias }, function (returnedData) {
            ...
            $("#subcats-box").toggle(false);
            $("#filtro-sub-categorias").html('<small class="text-muted">Selecciona una categoría para ver subcategorías.</small>');
...
$(document).on('keydown', '#inputBusqueda', function (e) {
...
// ✅ REEMPLAZAR EL BUCLE DENTRO DE renderEmpresas
function renderEmpresas(dataset, page = 1) {
    ...
    const empresasPagina = empresasFiltradas.slice(start, start + itemsPerPage);

    let listHtml = "";
    empresasPagina.forEach(function (p) {
        const descuentoPorcentual = parseFloat(p.descuento || 0);
        const tieneDesc = descuentoPorcentual > 0; // ✅ Validación de descuento
        const precioRef = Number(p.precio_referencia) || 0;
        const pFinal = tieneDesc ? precioRef * (1 - descuentoPorcentual / 100) : precioRef;
        const titulo = p.nombre || p.titulo_producto || 'Producto';
        const tieneDescuento = parseFloat(p.descuento) > 0;
        const descuento = Number(p.descuento) || 0;
        const precioDescuento = precioRef - (precioRef * descuento / 100);
        const verificado = Array.isArray(p.verificacion) && p.verificacion.length > 0 && Number(p.verificacion[0].verificado) === 1;

        listHtml += `
    <div class="product-exclusive-card shadow-sm">
        <div class="product-img-container" onclick="irADetalleProductoConTerminos(${p.id_producto})">
            <img src="../admin/${p.img_frontal}" 
                 onerror="this.src='../img/FULMUV_LOGO-13.png';" 
                 class="img-main">
            ${verificado ? `<img src="../img/verificado_empresa.png" alt="Vendedor verificado" class="product-verify-badge">` : ''}
            ${tieneDesc ? `<div class="exclusive-badge-discount">-${parseInt(descuentoPorcentual)}%</div>` : ''}
        </div>
        <div class="product-details-exclusive">
            <div>
                <h6 class="product-title-exclusive">${capitalizarPrimeraLetra(titulo)}</h6>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-end mt-2">
                ...
                <button class="btn-add-exclusive-circle" onclick="irADetalleProductoConTerminos(${p.id_producto})">
    ...
    $("#countVendedores").text(`Encontramos ${empresasFiltradas.length} artículos`);
    updateResultsButtonCount(empresasFiltradas.length);
    updateFilterActiveCount();

    // ✅ Cambiamos .append por .html para limpiar correctamente la página
    $("#listaProductosContainer").html(listHtml || '<div class="col-12 text-center py-5">No hay productos.</div>');
...
function renderCheckboxMarcas(marcas) {
    let html = `
      <div class="form-check">
        <input class="form-check-input chk-marca" type="checkbox" id="marca-all" value="__all__"
          ${marcasSeleccionadas.size === 0 ? 'checked' : ''}>
        <label class="form-check-label fw-normal" for="marca-all">
          Todos
        </label>
      </div>`;
    (marcas || []).forEach(m => {
...
function renderCheckboxModelos(modelos) {
    let html = `
      <div class="form-check">
        <input class="form-check-input chk-modelo" type="checkbox" id="modelo-all" value="__all__"
          ${modelosSeleccionados.size === 0 ? 'checked' : ''}>
        <label class="form-check-label fw-normal" for="modelo-all">
          Todos
        </label>
      </div>`;
    (modelos || []).forEach(m => {
...
function renderCheckboxCategorias(categorias) {
    let html = `
            <div class="form-check">
                <input type="checkbox" class="form-check-input" value="__all__" 
                       id="categoria-all" autocomplete="off" name="checkbox" ${subcategoriasSeleccionadas.length === 0 ? 'checked' : ''}>
                <label class="form-check-label fw-normal" for="categoria-all">
                    Todos
                </label>
            </div>`;
        ...
        html += `
            <div class="form-check">
                <input type="checkbox" class="form-check-input" value="${id}" 
                       id="categoria-${id}" autocomplete="off" name="checkbox" ${checked ? 'checked' : ''}>
                <label class="form-check-label fw-normal" for="categoria-${id}">
                    ${capitalizarPrimeraLetra(cat.nombre)}
                </label>
            </div>`;
    });
    $("#filtro-categorias").html(html || `<small class="text-muted">No hay categorías.</small>`);
}
...
function renderCheckboxSubcategorias(subcats) {
    ...
    const htmlFinal = `${htmlVisible}${htmlOcultas}` || '<small class="text-muted">No hay sub-categorías para las categorías seleccionadas.</small>';
...
function actualizarUIUbicacionPersistir() {
    ...
    if (btn) btn.innerHTML = `<i class="fi-rs-marker"></i> ${labelUbicacion()}`;
...
function irADetalleProductoConTerminos(id_producto) {
    ...
    window.location.href = `${detailPath}?q=${id_producto}`;
}
...
function openFilterPanel() {
    setActiveFilterSection(activeFilterSection || "categorias");
...
function updateFilterActiveCount() {
    const ubicacionOrdenCount =
        (provinciaSel.id ? 1 : 0) +
        (cantonSel.id ? 1 : 0) +
        (sortOption !== "todos" ? 1 : 0);
    const categoriasCount = subcategoriasSeleccionadas.length;
    const subcategoriasCount = subcategoriasHijasSeleccionadas.length;
    const marcasCount = marcasSeleccionadas.size;
    const modelosCount = modelosSeleccionados.size;
    const total = ubicacionOrdenCount + categoriasCount + subcategoriasCount + marcasCount + modelosCount;

    $("#filterActiveCount").text(total);
    $("#filterGroupCountUbicacionOrden").text(ubicacionOrdenCount);
    $("#filterGroupCountCategorias").text(categoriasCount);
    $("#filterGroupCountSubcategorias").text(subcategoriasCount);
