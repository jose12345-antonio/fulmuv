...
<style>
    ...
    .app-filter-search-input::placeholder {
        color: #94a3b8;
    }

    .app-filter-search-empty {
        display: none;
        padding: 10px 12px;
        border-radius: 12px;
...
<script>
    ...
    function normalizeAppFilterSearchText(value) {
        return String(value || "")
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .toLowerCase()
    ...
    function getDirectFilterHost(container, node) {
        let current = node;
    ...
    function getFilterOptionText(container, input, host) {
        const inputId = input?.id || "";
        const explicitLabel = inputId ? container.querySelector(`label[for="${inputId}"]`) : null;
        const sourceText = explicitLabel ? explicitLabel.textContent : host.textContent;
        return normalizeAppFilterSearchText(sourceText);
    }

    function getFilterOptionHosts(container) {
        const inputs = Array.from(container.querySelectorAll('input[type="checkbox"], input[type="radio"]'))
            .filter(input => !input.closest(".app-filter-search-wrap"));

        const hosts = new Map();

        inputs.forEach(input => {
            const optionNode = input.closest(".form-check, label, .filter-check-item, .filter-radio-item, .category-chip-card, .filter-choice-card") || input;
            const host = getDirectFilterHost(container, optionNode);
            if (!host) return;

            const key = host.dataset.filterSearchKey || input.id || `${input.name}_${input.value}`;
            host.dataset.filterSearchKey = key;

            hosts.set(key, {
                host,
                input,
                text: getFilterOptionText(container, input, host),
                alwaysVisible: input.value === "__all__"
            });
        });
    ...
    function filterAppFilterOptions(container, term) {
        const normalizedTerm = normalizeAppFilterSearchText(term);
        const options = getFilterOptionHosts(container);
        let visibleCount = 0;

        options.forEach(({ host, text, alwaysVisible }) => {
            const visible = !normalizedTerm || alwaysVisible || text.includes(normalizedTerm);
            host.style.display = visible ? "" : "none";
            if (visible) visibleCount += 1;
        });

        const emptyState = container.querySelector(".app-filter-search-empty");
        if (emptyState) {
            emptyState.style.display = visibleCount === 0 ? "block" : "none";
        }
    }

    function enhanceAppFilterContainer(container) {
        if (!container || container.dataset.filterSearchReady === "1") return;

        const options = getFilterOptionHosts(container);
        if (!options.length) return;

        const currentValue = container.dataset.filterSearchValue || "";
        const searchWrap = document.createElement("div");
        searchWrap.className = "app-filter-search-wrap";
        searchWrap.innerHTML = `
            <input type="text" class="app-filter-search-input" placeholder="Buscar en este filtro...">
            <div class="app-filter-search-empty">No encontramos opciones con ese término.</div>
        `;

        container.prepend(searchWrap);

        const input = searchWrap.querySelector(".app-filter-search-input");
        input.value = currentValue;
        input.addEventListener("input", function() {
            container.dataset.filterSearchValue = this.value;
            filterAppFilterOptions(container, this.value);
        });

        container.dataset.filterSearchReady = "1";
        filterAppFilterOptions(container, currentValue);
    }

    function resetAppFilterSearchMarker(container) {
        if (!container) return;

        const hasOwnSearch = container.firstElementChild && container.firstElementChild.classList.contains("app-filter-search-wrap");
        if (!hasOwnSearch) {
            delete container.dataset.filterSearchReady;
        }
    }

    function refreshAppFilterSearches() {
        document.querySelectorAll('[id^="filtro-"]').forEach(container => {
            resetAppFilterSearchMarker(container);
            enhanceAppFilterContainer(container);
        });
    }

    let appFilterRefreshQueued = false;
    function queueAppFilterSearchRefresh() {
        if (appFilterRefreshQueued) return;
        appFilterRefreshQueued = true;
        requestAnimationFrame(() => {
            appFilterRefreshQueued = false;
            refreshAppFilterSearches();
        });
    }

    window.refreshAppFilterSearches = queueAppFilterSearchRefresh;

    $(document).ready(function() {
        queueAppFilterSearchRefresh();
