...
function registrarInteraccionProducto(tipoEvento, payload = {}) {
    ...
    $.ajax({
        url: "../api/track_producto_interacción.php",
        method: "POST",
        dataType: "json",
        data: {
        ...
        if (!returnedData.error) {
            ...
            $("#contactosEmpresa").html(`
                <div class="d-flex flex-wrap gap-2 mt-2">
                    ${telefonoDigits ? `
                        <a href="tel:${telefonoDigits}" class="btn btn-sm btn-outline-primary" id="btnTelefonoEmpresa">
                            <i class="fi-rs-phone-call me-1"></i> Llamar al telefono
                        </a>
                    ` : ""}
                </div>
            `);
