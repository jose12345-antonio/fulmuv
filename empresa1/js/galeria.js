  ...
  $.get('../api/v1/fulmuv/fileEmpresaById/' + id_archivo_empresa, {}, function (returnedData) {
              ...
              <div class="p-4">
                <div class="row g-3">
                  <div class="col-md-7">
                    ...
                    <textarea class="form-control" id="edit_descripcion" maxlength="300">${$('<div>').text(descripcion).html()}</textarea>
                  </div>
                  <div class="col-md-5">
                    ...
                    <div class="small text-muted mt-2">Archivo actual</div>
                  </div>
                  <div class="col-12 d-flex gap-2">
                    <button class="btn btn-primary" id="btnUpdateGaleria">Actualizar</button>
    ...
    $(document).off('click', '#btnUpdateGaleria').on('click', '#btnUpdateGaleria', function(){
      ...
      $.post('../api/v1/fulmuv/updateFileEmpresa', {
        ...
        if (resp.error === false) {
          // si el botón/target tiene data-file-id, refrescamos la tarjeta sin recargar
          var $card = $(`[data-file-id="${id_archivo_empresa}"]`).closest('.gallery-card, .border, .card');
          if ($card.length) {
            $card.find('.gallery-title,.fs-9 h5,.fs-9,.card-title').text(tituloN);
            $card.find('.gallery-desc,.fs-10,.card-text').text(descN);
          }
          SweetAlert("success", resp.msg || "Actualizado correctamente");
          // cerrar modal
  ...
  if (titulo == "" || descripcion == "") {
    ...
    filePromise.then(function (file) {
      $.post('../api/v1/fulmuv/createFileEmpresa', {
        ...
        if (returned.error == false) {
          SweetAlert("url_success", returned.msg, "galeria.php");
        } else {
          SweetAlert("error", returned.msg);
        }
      });
            ...
            <div class="p-4">
              <div class="row g-2">
                <div class="col-md-12 mb-3">
                  ...
                  <input class="form-control" id="titulo" type="text" placeholder="Ingrese título"/>
                </div>
                <div class="col-md-12 mb-3">
                ...
                <div class="col-12">
                  <button onclick="registrarImagen()" class="btn btn-iso" type="button">Guardar</button>
                </div>
              </div>
            </div>
          </div>
