  ...
  $.get('../api/v1/fulmuv/servicios/all/' + id_empresa + '/' + tipo_user, {}, function (returnedData) {
    ...
    if (returned.error == false) {
      ...
      returned.data.forEach(producto => {
        // 1. Extraer la categoría de forma segura
        const nombreCat = (producto.categorias && producto.categorias.length > 0)
          ? producto.categorias[0].nombre
          : 'Sin categoría';

        // 2. Formatear los tags como pequeñas etiquetas (badges)
        const tagsHtml = producto.tags
          ? producto.tags.split(',').map(tag => `<span class="badge badge-subtle-primary fs-11">${tag.trim()}</span>`).join(' ')
          : '';

        // 3. Determinar la imagen a mostrar
        const imagenUrl = producto.img_frontal
          ? "../admin/" + producto.img_frontal
        ...
        <tr class="btn-reveal-trigger">
            <td class="align-middle white-space-nowrap">
                <div class="d-flex align-items-center position-relative">
                    <div class="avatar avatar-xl img-thumbnail shadow-sm me-3">
                        <img class="rounded-soft" src="${imagenUrl}" alt="${producto.titulo_producto}" />
                    </div>
                    <div class="flex-1">
                        <h6 class="mb-0 fw-bold">
                            <a class="text-900 stretched-link" href="crear_servicio.php?id_producto=${producto.id_producto}">
            ...
            <td class="align-middle text-700 fw-medium py-2">
                <span class="fs-9 text-dark">$${parseFloat(producto.precio_referencia).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
            </td>
            <td class="align-middle white-space-nowrap">
                <span class="badge rounded-pill badge-subtle-success"><span class="fas fa-tag me-1"></span>${nombreCat}</span>
            </td>
            <td class="align-middle white-space-nowrap py-2 text-end">
                <div class="dropdown font-sans-serif position-static">
                    ...
                    <div class="dropdown-menu dropdown-menu-end border py-0">
                        <div class="py-2">
                            <a class="dropdown-item" href="crear_servicio.php?id_producto=${producto.id_producto}">
                                <span class="fas fa-edit me-2"></span>Editar
                            </a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item text-danger" onclick="remove(${producto.id_producto}, 'productos')">
                                <span class="fas fa-trash-alt me-2"></span>Eliminar
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-content border-0">
              ...
              <div class="modal-body p-0">
                <div class="rounded-top-3 bg-body-tertiary py-3 ps-4 pe-6">
                  <h4 class="mb-1" id="staticBackdropLabel">Actualizar servicio</h4>
                </div>
                <div class="p-4">
                  <div class="row g-2">
                    <div class="col-6 mb-2">
                      <label class="form-label" for="nombre">Nombre servicio:</label>
