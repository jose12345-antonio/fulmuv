...
$(document).ready(function () {
  ...
  $.get('../api/v1/fulmuv/getReferencias/', {}, function (r) {
    ...
    if (!d.error) d.data.forEach(ref => $("#referencia").append(`<option value="${ref}">${ref}</option>`));
  });
  ...
  $.get('../api/v1/fulmuv/tiposAuto/', {}, function (r) {
    ...
    if (!d.error) {
      tipos_auto = d.data;
      d.data.forEach(t => $("#tipo_vehiculo").append(`<option value="${t.id_tipo_auto}">${t.nombre}</option>`));
      $("#tipo_vehiculo").select2({
        theme: 'bootstrap-5', tags: true, placeholder: 'Seleccione tipo de vehículo', allowClear: true,
  ...
  $.get('../api/v1/fulmuv/marcas/', {}, function (r) {
    ...
    if (!d.error) {
      ...
      $("#marca").select2({
        theme: 'bootstrap-5', tags: true, placeholder: 'Seleccione marca', allowClear: true,
        createTag: function (params) {
          var term = $.trim(params.term).toUpperCase();
          if (!term) return null;
          return { id: 'nuevo', text: term, newTag: true };
  ...
  $.get('../api/v1/fulmuv/tipo_tracccion/', {}, function (r) {
    ...
    if (!d.error) {
      ...
      $("#traccion").select2({
        theme: 'bootstrap-5', tags: true, placeholder: 'Seleccione tracción', allowClear: true,
        createTag: function (params) {
          var term = $.trim(params.term).toUpperCase();
          if (!term) return null;
          return { id: 'nuevo', text: term, newTag: true };
  ...
  $.get('../api/v1/fulmuv/getFuncionamientoMotor/', {}, function (r) {
    ...
    if (!d.error) {
      ...
      $("#motor").select2({
        theme: 'bootstrap-5', tags: true, placeholder: 'Seleccione funcionamiento de motor', allowClear: true,
        createTag: function (params) {
          var term = $.trim(params.term).toUpperCase();
          if (!term) return null;
          return { id: 'nuevo', text: term, newTag: true };
...
function cargarVehiculoParaEditar(id){
  $.ajax({
    ...
    success: function(r){
      ...
      // ✅ Cargar Referencia y Modelos
      const refs = safeArr(v.referencias);
      setSelect2ValueByText($("#referencia"), refs);

      const modelosPairs = normMulti(v.id_modelo, 'id_modelos_autos', 'nombre');
      buscarModelosReferencia(refs[0] || '', modelosPairs);

      // Cargar dependencias de Modelo (se mantienen guardadas)
      $("#tipo_vehiculo").val(String(v.tipo_auto || '')).trigger("change");
      $("#marca").val(String(v.id_marca || '')).trigger("change");
      ...
      // Apagar seguro
      setTimeout(() => { isEditLoading = false; }, 500);
    }
  });
}
...
function buscarModelosReferencia(referenciaParam = null, modelosPairs = []){
  ...
  $.get('../api/v1/fulmuv/getModelosByReferencia/' + encodeURIComponent(referencia), {}, function (returnedData) {
    ...
    $sel.select2({
      theme: 'bootstrap-5', tags: true, placeholder: 'Seleccione modelo', allowClear: true,
      createTag: function (params) {
        var term = $.trim(params.term).toUpperCase();
        if (!term) return null;
        return { id: 'nuevo', text: term, newTag: true };
    ...
    // Seleccionar modelos pre-existentes al editar
...
function asignarModelo(){
  ...
  $.get('../api/v1/fulmuv/getModeloById/' + id_modelo, {}, function (returnedData) {
    ...
    const d = r.data;
    $("#marca").val(d.id_marca).trigger("change");
    $("#tipo_vehiculo").val(d.id_tipo_auto).trigger("change");
    $("#traccion").val(d.id_tipo_traccion).trigger("change");
    $("#motor").val(d.id_funcionamiento_motor).trigger("change");
  });
}
...
function validarCamposObligatorios() {
  ...
  if (document.querySelector("#descripcion")) {
    ...
    const c = (ed ? ed.getContent({ format: "text" }) : $("#descripcion").val()).trim();
    if (ed) ed.save(); 
...
function verificarMembresiaYGuardar() {
  ...
  $.get('../api/v1/fulmuv/validarMembresiaProductos/' + id_empresa + '/' + tipo_user, function (data) {
    const res = typeof data === 'string' ? JSON.parse(data) : data;
    if (res.error) swal({ title: "Mejora tu plan", text: res.msg, icon: "info" }, () => window.location.href="upgrade_membresia.php?id="+id_empresa);
    else addProducto();
  });
}
