  ...
  subirImagenesPrincipales().then(imagenes => {
    saveFiles(files).then(archivos => {
      ...
      $.post('../api/v1/fulmuv/empleos/create', {
        ...
        if (!r.error) SweetAlert("url_success", r.msg, "crear_empleo.php");
        else SweetAlert("error", r.msg || "Error al crear.");
      });
  ...
  subirImagenesPrincipalesEdit().then(imagenes => {
    saveFilesEdit(dz).then(archivosNuevos => {
      ...
      $.post('../api/v1/fulmuv/empleos/update', {
        ...
        const r = (typeof resp === 'string') ? JSON.parse(resp) : resp;
        if (!r.error) SweetAlert("url_success", r.msg || "Actualizado.", "crear_empleo.php");
        else SweetAlert("error", r.msg || "Error al actualizar.");
      });
