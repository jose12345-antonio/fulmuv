...
// ✅ Formateador de rutas para previsualización
...
$(document).ready(function () {
  ...
  if (id_producto != "") {
    $.get(`../api/v1/fulmuv/productos/${id_producto}`, function (respRaw) {
      ...
      setTinyMCE('descripcion', p.descripcion || "");
      ...
      // === Nombre de servicio
      if (p.id_nombre_servicio) {
        setSelectValue($("#nombre"), p.id_nombre_servicio);
      } else if (p.nombre) {
        setSelectValue($("#nombre"), p.nombre, p.nombre);
      }
      ...
      setTimeout(() => { isEditLoading = false; }, 300);
  ...
  subirImagenesPrincipales()
    ...
    .then(({ imagenes, archivos }) => {
      ...
      return $.post('../api/v1/fulmuv/productos/create', {
        ...
        codigo: '',
        categoria: categoria,
        sub_categoria: [], // ✅ Siempre vacío para servicios
        tags: tags.join(', '),
    ...
    .done(function (returnedData) {
      ...
      if (returned.error == false) {
        SweetAlert("url_success", returned.msg, "servicios.php"); // ✅ Redirige a servicios
      } else {
...
function editProducto(btnSelector) {
  ...
  subirImagenesPrincipalesEdit()
    ...
    .then(({ imagenes, archivosNuevos }) => {
      // Retornamos el $.post para que la cadena continúe con el éxito del servidor
      return $.post('../api/v1/fulmuv/productos/edit', {
        nombre: texto,
        descripcion: descripcion,
        codigo: '',
        categoria: categoria,
    ...
    .then(returnedData => { // Cambiado de .done() a .then()
    ...
    .catch(err => { // Cambiado de .fail() a .catch() para capturar errores de Promesas
      console.error(err);
      SweetAlert("error", "Ocurrió un error al procesar la edición.");
    })
    .finally(() => { // Cambiado de .always() a .finally()
      if (btnSelector) resetBtnLoading(btnSelector);
    });
}
