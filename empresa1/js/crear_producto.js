    ...
    $.get(`../api/v1/fulmuv/productos/${id_producto}`, function (respRaw) {
      ...
      if (myDropzone && Array.isArray(archivosGaleria)) {
        ...
        archivosGaleria.forEach(function (arch) {
          const ruta = "../admin/" + arch.archivo;

          // 1. Creamos el objeto mock
          const mockFile = {
            name: ruta.split('/').pop(),
            size: 123456,
            accepted: true,
            existing: true,
            id_archivo_producto: arch.id_archivo_producto,
            status: Dropzone.SUCCESS
          };

          // 2. Usamos displayExistingFile (esto ya dispara los eventos necesarios internamente)
          myDropzone.displayExistingFile(mockFile, ruta);

          // 3. Vinculamos el evento de eliminar al elemento que se acaba de crear
          const $previewElement = $(mockFile.previewElement);

          // Selector para el link de eliminar que definiste en tu HTML
          $previewElement.find('[data-dz-remove]').on("click", function (e) {
            e.preventDefault();
            e.stopPropagation();
...
function buscarModelosReferencia(refs, modelosSeleccionar = []) {
  return new Promise((resolve, reject) => {
    ...
    $.get('../api/v1/fulmuv/getModelosByReferencia/' + referencia, {}, function (returnedData) {
      let returned = (typeof returnedData === "string") ? JSON.parse(returnedData) : returnedData;
...
function addProducto() {
  ...
  // CORRECCIÓN AQUÍ: Estaba mal cerrado y con un return suelto
...
function eliminarArchivoGaleria(id_archivo_producto, file) {
  swal({
    ...
    text: "Esta acción borrará el archivo permanentemente de la base de datos.",
    type: "warning",
    showCancelButton: true,
    confirmButtonColor: "#DD6B55",
    confirmButtonText: "Sí, eliminar",
    cancelButtonText: "Cancelar",
    closeOnConfirm: false // Mantenemos abierto para mostrar el éxito/error
  }, function (isConfirm) {
    if (isConfirm) {
      $.post('../api/v1/fulmuv/archivos_productos/delete', {
        id_archivo_producto: id_archivo_producto
      }, function (respRaw) {
        const resp = (typeof respRaw === 'string') ? JSON.parse(respRaw) : respRaw; 

        if (!resp.error) {
          // Eliminar visualmente del Dropzone
          if (myDropzone) {
            myDropzone.removeFile(file);
          }

          // Notificación de éxito con la función que ya tienes
          SweetAlert("success", resp.msg || "Archivo eliminado correctamente.");
        } else {
          SweetAlert("error", resp.msg || "No se pudo eliminar el archivo.");
        }
      }).fail(function () {
        SweetAlert("error", "Error de red al intentar eliminar el archivo.");
      });
    }
  });
}
...
function editProducto() {
  ...
  if (!validarCamposObligatorios()) {
    ...
    subirImagenesPrincipalesEdit().then(imagenes => {
      setBtnLoading("#btnActualizarProducto", true, "Guardando cambios...");
