...
async function addRowWithDataVehiculo(v){
  ...
  setSelectSingle(tr.querySelector(".tipo_auto"), v.tipo_vehiculo || "");
...
function getRowsData(){
  ...
  rows.forEach((tr, index) => {
    ...
    out.push({
      ...
      modelo: tr.querySelector(".id_modelo").value,
      tipo_vehiculo: tr.querySelector(".tipo_auto").value,
      id_marca: tr.querySelector(".id_marca").value,
      tipo_traccion: tr.querySelector(".tipo_traccion").value,
      motor: tr.querySelector(".funcionamiento_motor").value,
      ...
      duenio: tr.querySelector(".tipo_duenio").value,
...
function validarFila(v){
  ...
  if (isEmptyStr(v.tipo_vehiculo)) errores.push(`Fila ${fila}: Falta tipo auto`);
  ...
  if (isEmptyStr(v.motor)) errores.push(`Fila ${fila}: Falta funcionamiento motor`);
  if (isEmptyStr(v.modelo)) errores.push(`Fila ${fila}: Falta modelo`);
...
async function guardarMasivo(estado) {
  ...
  const isRowEmpty = (v) => {
    ...
    const hasSelects = !!String(v.modelo || "").trim() || !!String(v.id_marca || "").trim() || !!String(v.tipo_vehiculo || "").trim() || !!String(v.tipo_traccion || "").trim() || !!String(v.funcionamiento_motor || "").trim();
