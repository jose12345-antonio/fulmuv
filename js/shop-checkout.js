...
let pickupSeleccionadas = new Set();
...
$(function () {
    ...
    function goStep2() {
        ...
        $s1Head.addClass('is-complete').removeClass('is-active');
        $s2Head.addClass('is-active');
        // Cambiar layout: col-12 → col-lg-8, mostrar resumen
        $('#checkoutFormCol').removeClass('col-12').addClass('col-lg-8');
        $('#checkoutSummaryCol').removeClass('d-none');
        modoEntrega = 1;
        applyModoEntregaUI();
    ...
    function goStep1() {
        ...
        $s2Head.removeClass('is-active is-complete');
        $s1Head.addClass('is-active').removeClass('is-complete');
        // Volver layout: col-lg-8 → col-12, ocultar resumen
        $('#checkoutFormCol').removeClass('col-lg-8').addClass('col-12');
        $('#checkoutSummaryCol').addClass('d-none');
        window.scrollTo({ top: 0, behavior: 'smooth' });
    }
    ...
    function validateStep2() {
        ...
        if ($('#btnEnvio').hasClass('active')) {
            ...
            const selectedCount = $('#pickupLista input.pickup-check:checked').length;
            if (selectedCount === 0) {
                ok = false;
                msg = 'Selecciona al menos una empresa para recoger en tienda.';
            }
        }
    ...
    $('#btnEnvio').on('click', function () {
        const totalEmpresas = $('#pickupLista input.pickup-check').length;
        const selectedCount = $('#pickupLista input.pickup-check:checked').length;
        if (totalEmpresas > 0 && selectedCount === totalEmpresas) {
            Swal.fire({
                icon: 'warning',
                title: 'Recogida seleccionada',
                text: 'Seleccionaste todas las empresas para recoger tu pedido en tienda. Para habilitar envío a domicilio, desmarca al menos una empresa.'
            });
            return;
        }
        $(this).addClass('active');
        $('#btnPickup').removeClass('active');
        $('#envioDomicilio').removeClass('d-none');
        $('#pickupTienda').addClass('d-none');
    });
...
function cargarUltimaFacturacion() {
    const idCliente = (typeof SESION_CLIENTE_ID !== 'undefined') ? SESION_CLIENTE_ID : 0;
    if (!idCliente) return;

    // Solo pre-llenar si el formulario está vacío (no sobreescribir lo que el usuario ya escribió)
    const yaEscrito = ['#fact_nombre', '#identificacion', '#correo', '#telefono_fact', '#direccion_fiscal']
        .some(sel => ($(sel).val() || '').trim() !== '');
    if (yaEscrito) return;

    $.post('api/v1/fulmuv/cliente/getUltimaFacturacion', { id_cliente: idCliente }, function (res) {
        if (!res.error && res.data) {
            const d = res.data;
            if (d.razon_social)         $('#fact_nombre').val(d.razon_social);
            if (d.tipo_identificacion)  $('#tipo_identificacion').val(d.tipo_identificacion);
            if (d.numero_identificacion) $('#identificacion').val(d.numero_identificacion);
            if (d.correo)               $('#correo').val(d.correo);
            if (d.telefono)             $('#telefono_fact').val(d.telefono);
            if (d.direccion)            $('#direccion_fiscal').val(d.direccion);
            if (d.forma_pago)           $('#forma_pago').val(d.forma_pago);
            saveIdent();
        }
    }, 'json');
}
...
$(function () {
    ...
    setTimeout(restoreEnvio, 100);

    $('#step1').on('input change', '#fact_nombre,#tipo_identificacion,#identificacion,#correo,#telefono_fact,#direccion_fiscal,#forma_pago', saveIdent);
...
function obtenerDireccionDesdeCoords(lat, lng, callback = null) {
    ...
    geocoder.geocode({ location: latlng }, (results, status) => {
        if (status === "OK" && results[0]) {
            ...
            const cp = extraerCodigoPostalDeComponents(results[0].address_components || []);
            setCodigoPostal(cp);
...
function actualizarshopCart() {
    ...
    carrito.forEach(item => {
        ...
        if (precioOriginal > precioConDesc) {
            htmlPrecio = `<div style="white-space:nowrap;">
                <div style="font-size:10px;text-decoration:line-through;color:#FF6D01;font-weight:400;">${formatPrecioSuperscript(precioOriginal)}</div>
                <div style="font-size:12px;color:#004E60;font-weight:800;">${formatPrecioSuperscript(precioConDesc)}</div>
            </div>`;
        } else {
            htmlPrecio = `<div style="font-size:12px;color:#004E60;font-weight:800;white-space:nowrap;">${formatPrecioSuperscript(precioOriginal)}</div>`;
        }

        const nombre = capitalizarPrimeraLetra ? capitalizarPrimeraLetra(item.nombre) : item.nombre;
        const badgeIVA = tieneIVA ? '<span class="co-badge-iva">IVA incl.</span>' : '';

        $("#coProductList").append(`
            <div class="co-mini-item">
                <img src="${item.imagen}" onerror="this.src='img/FULMUV-NEGRO.png';" class="co-mini-img">
                <div class="co-mini-info">
                    <div class="co-mini-name">${nombre}</div>
                    <div class="co-mini-qty">× ${cantidad} ${badgeIVA}</div>
                </div>
                <div class="co-mini-price">${htmlPrecio}</div>
            </div>
        `);
    });
...
async function generarOrden() {
    ...
    try {
        ...
        const selectedPickup = getSelectedPickupIds();
        const ordenes_empresas = Object.entries(productosAgrupados)
            .map(([id_empresa, productos]) => ({
                id_empresa,
                productos,
                total: productos.reduce((acc, p) => acc + p.precio * p.cantidad, 0),
                envio_domicilio: selectedPickup.has(String(id_empresa)) ? 1 : 0
            }));

        const datos = {
            numero_orden: "ORD-" + Date.now(),
            id_cliente: (typeof SESION_CLIENTE_ID !== 'undefined') ? SESION_CLIENTE_ID : 0,
            ...
            datos_facturacion: {
                ...
                telefono: telefono_fact, direccion: direccion_fiscal, forma_pago
            },
...
function renderListaPuntosRecogida(empresas) {
    ...
    empresas.forEach(e => {
        ...
        const isChecked = pickupSeleccionadas.has(String(id));
        $list.append(`
      <label class="list-group-item d-flex justify-content-between align-items-start">
        <div class="form-check">
          <input class="form-check-input me-2 pickup-check" type="checkbox" name="pickupSelect" value="${id}" ${isChecked ? 'checked' : ''}
...
function getSelectedPickupIds() {
    return new Set($('#pickupLista input.pickup-check:checked').map(function () { return String($(this).val()); }).get());
}

function updatePickupRules() {
    const totalEmpresas = $('#pickupLista input.pickup-check').length;
    const selectedCount = $('#pickupLista input.pickup-check:checked').length;
    const allSelected = totalEmpresas > 0 && selectedCount === totalEmpresas;

    const $msg = $("#pickupMensajeRegla");
    if (allSelected) {
        $("#btnEnvio").prop("disabled", true).addClass("disabled").attr("aria-disabled", "true");
        $("#btnEnvio").css("pointer-events", "none");
        $msg.removeClass("d-none").html(`
          Has seleccionado recoger en tienda para <strong>todas</strong> las empresas de tu carrito.
          Para habilitar envío a domicilio, desmarca al menos una empresa.
        `);
    } else {
        $("#btnEnvio").prop("disabled", false).removeClass("disabled").removeAttr("aria-disabled");
        $("#btnEnvio").css("pointer-events", "");
...
$(document).on("change", 'input[name="pickupSelect"]', function () {
    ...
    if ($(this).is(':checked')) pickupSeleccionadas.add(id);
    else pickupSeleccionadas.delete(id);
    updatePickupRules();
});
