...
$(document).ready(function () {
    ...
    $.get("api/v1/fulmuv/productos/" + id_producto, function (returnedData) {
        ...
        if (!returnedData.error) {
            ...
            // MARCA(S)
            const marcasTxt = returnedData.data.marca_producto;
            ...
            const modelosTxt = joinNombres(modelosRaw);
            $("#modeloProducto").text(modelosTxt);

            // PESO
            $("#pesoProducto").text(`${returnedData.data.peso} ${returnedData.data.unidad || "kg"}`);
            ...
            <div class="contact-line">
                ...
                <span>Teléfono:</span>
                ${telDigits ? `<a href="${telLink}">${prettyPhone(telRaw)}</a>` : `<span class="text-muted">No disponible</span>`}
            </div>
            ...
            <div class="contact-line">
                ...
                <span>WhatsApp:</span>
                ${waDigits ? `<a href="${waChat}" target="_blank" rel="noopener">${prettyPhone(waRaw)}</a>` : `<span class="text-muted">No disponible</span>`}
            </div>
            `);
        ...
        `);
            ...
            $.post("api/v1/fulmuv/categorias/productos", {
                ...
                if (!returnedDataProducto.error && returnedDataProducto.data.length > 0) {
                    ...
                    // ✅ INICIALIZAR SLICK FUERA DEL BUCLE
                    container.slick({
                        dots: false,
                        infinite: true,
                        speed: 1000,
            ...
            $("#descripcionEmpresa").text(returnedData.data.empresa.tipo_parqueadero)
...
function setMapCoords(lat, lng) {
    // 👇 importante: normaliza coma decimal si viniera " -2,2758 "
