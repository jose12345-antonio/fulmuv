    ...
    $.post("api/v1/fulmuv/getOrdenesSeguimiento", { numero_orden }, function (res) {
        ...
        data.empresas.forEach(empresa => {
            try {
                ...
                const nombreTrayecto = !esRetiroEmpresa && empresa.trayecto && empresa.trayecto[0]
                    ? (empresa.trayecto[0].nombre || "Sin nombre")
                    : "";
                ...
                productos.forEach(p => {
                    ...
                    if (Number(p.iva) === 1) {
                        const base = linea / 1.15;
                        const iva = linea - base;
                        subtotalProductos += base;
                        totalIVA152 += iva;
                    } else {
                        subtotalProductos += linea;
                        totalIVA152 += linea * 0.15;
                    }
                });
                    ...
                    <div class="border rounded-3 p-3 mb-3 bg-white">
                      <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                        <div class="me-2">
                          ...
                          &nbsp;—&nbsp;<span class="text-muted">Empresa:</span> ${nombreEmpresa}
                          ${esRetiroEmpresa
                            ? `&nbsp;—&nbsp;<span class="text-muted">Retiro:</span> Retira tu pedido en la empresa <strong>${nombreEmpresa}</strong>.`
                            : `&nbsp;—&nbsp;<span class="text-muted">Trayecto:</span> ${nombreTrayecto}`
                          }
                        </div>
                        <div class="d-flex flex-wrap align-items-center gap-2 ms-auto">
                          <div class="border rounded-3 px-3 py-2 bg-light text-end">
                ...
                productos.forEach(p => {
                    productosHTML += `
                        ...
                        <td>${(typeof capitalizarPrimeraLetra === 'function' ? capitalizarPrimeraLetra(p.nombre) : (p.nombre || ''))}
                            ${Number(p.iva) === 1 ? '<span class="badge bg-success ms-2">IVA incluido</span>' : ''}
        ...
        const tieneEnvioDomicilio = (data.empresas || []).some(e => Number(e.envio_domicilio) === 0);
        if (!tieneEnvioDomicilio) {
            ...
            $("#address-tab").removeClass("d-none");
            $("#address").removeClass("d-none");
        }

        // Dirección
        if (tieneEnvioDomicilio && data.domicilio) {
            ...
            <div class="row g-3">
              <div class="col-md-6">
                <div class="border rounded-3 p-3 h-100 bg-light">
                  <div class="small text-muted">Razón social</div>
                  <div class="fw-bold">${data.facturacion.razon_social}</div>
                  <hr class="my-2">
                  <div class="small text-muted">Número de identificación</div>
                  <div class="fw-semibold">${data.facturacion.numero_identificacion}</div>
                  <hr class="my-2">
                  <div class="small text-muted">Correo electrónico</div>
                  <div class="fw-semibold">${data.facturacion.correo}</div>
                </div>
              </div>
              <div class="col-md-6">
                <div class="border rounded-3 p-3 h-100 bg-light">
                  <div class="small text-muted">Dirección fiscal</div>
                  <div class="fw-semibold">${data.facturacion.direccion}</div>
                  <hr class="my-2">
                  <div class="small text-muted">Teléfono</div>
                  <div class="fw-semibold">${data.facturacion.telefono}</div>
                  <hr class="my-2">
                  <div class="small text-muted">Forma de pago</div>
                  <div class="fw-semibold">${data.facturacion.forma_pago}</div>
                </div>
              </div>
            </div>
          `);
