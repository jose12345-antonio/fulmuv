          ...
          if (returned.error == false) {
            ...
            returned.data.forEach(orden => {
              ...
              switch (orden.orden_estado) {
                case "creada":
                  estado = "creada<span class='ms-1 fas fa-shopping-cart' data-fa-transform='shrink-2'></span>"
                  opciones_estados += `
                  ...
                  break;
                case "aprobada":
                  estado = "aprobada<span class='ms-1 fas fa-user-check' data-fa-transform='shrink-2'></span>"
                  
                  if (orden.envio_domicilio == 1) {
                    opciones_estados += `
                  ...
                  break;
                case "procesada":
                  estado = "procesada<span class='ms-1 fas fas fa-cogs' data-fa-transform='shrink-2'></span>"
                  opciones_estados += `
                    <a class="dropdown-item" onclick="updateEstado('enviada', [${orden.id_orden}], '${orden.orden_estado}')">Enviar</a>
                    <div class="dropdown-divider"></div>
                    <a class="dropdown-item text-danger" onclick="deleteOrden([${orden.id_orden}])">Eliminar</a>
                  `
                  break;
                case "enviada":
                  estado = "enviada<span class='ms-1 fas fas fa-truck' data-fa-transform='shrink-2'></span>"
                  opciones_estados += `
                    <a class="dropdown-item" onclick="updateEstado('completada', [${orden.id_orden}], '${orden.orden_estado}')">Completar</a>
                    <div class="dropdown-divider"></div>
                    <a class="dropdown-item text-danger" onclick="deleteOrden([${orden.id_orden}])">Eliminar</a>
                  `
                  break;
                case "completada":
                  estado = "completada<span class='ms-1 fas fas fa-check' data-fa-transform='shrink-2'></span>"
                  break;
                default:
                  break;
              }
                ...
                <tr class="btn-reveal-trigger">
                  <td class="align-middle" style="width: 28px;">
                    <div class="form-check mb-0">
                      <input class="form-check-input" type="checkbox" id="number-pagination-item-${orden.id_orden}" data-id="${orden.id_orden}" data-estado="${orden.orden_estado}" data-envio="${orden.envio_domicilio}" data-bulk-select-row="data-bulk-select-row" />
                    </div>
                  </td>
                  ...
                  <td class="align-middle white-space-nowrap">${orden.created_at}</td>
                  <td class="align-middle white-space-nowrap">
                    <div class="fw-semi-bold">${clienteNombre || '-'}</div>
                    ${clienteCedula}
                    ${clienteTelefono}
                    ${clienteCorreo}
                  </td>
                  ...
                  <!--td class="align-middle white-space-nowrap">
                    <div class="btn-group"> 
                      <button type="button" class="d-inline-flex btn bg-200 fs-10">${estado}</button>
                      <button type="button" class="btn bg-200 dropdown-toggle dropdown-toggle-split maind-btn border-start" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                  ...
                  <td class="align-middle amount">$${orden.total}</td>
