...
function outlineBadges(values) {
  ...
  return list.map((v, index) => `${index ? '<span class="catalog-separator">,</span>' : ''}<span class="catalog-outline-badge">${v}</span>`).join(' ');
}
...
function renderVehiculosCards(items, page = 1) {
  ...
  slice.forEach((vehiculo) => {
    ...
    const specsHtml = outlineBadges([
      ...parseArrayish(vehiculo.condicionArray || vehiculo.condicion),
      ...parseArrayish(vehiculo.transmisionArray || vehiculo.transmision),
      ...parseArrayish(vehiculo.tipo_vendedorArray || vehiculo.tipo_vendedor)
      ...
      <div class="col-md-6 col-xl-4 mb-3">
        <div class="catalog-card">
          <div class="catalog-media">
            ...
            <img src="${imagenUrl}" alt="${vehiculo.modelo || 'Vehículo'}" />
          </div>
          <div class="catalog-body">
            <div class="d-flex align-items-start justify-content-between gap-2 mb-1">
              <div class="catalog-title">${vehiculo.modelo || 'Vehículo sin modelo'}</div>
              <span class="badge rounded-pill badge-subtle-success">${vehiculo.provincia || 'Sin provincia'}</span>
            </div>
            <div class="catalog-meta mb-1">${vehiculo.canton || 'Sin cantón'}</div>
            <div class="d-flex flex-wrap align-items-center gap-1 mb-1">
              ${specsHtml}
            </div>
            <div class="d-flex align-items-center justify-content-between gap-2 mb-2">
              <div class="catalog-price">$${formatMoney(vehiculo.precio_referencia)}</div>
              <div class="catalog-meta text-end">${vehiculo.anio || 'Sin año'}</div>
            </div>
