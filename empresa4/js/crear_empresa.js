  ...
  let responseCallback = response => {
    ...
    if (response.card) {
      ...
      if (response.card.status == "valid") {
        ...
        completarRegistroEmpresaConCobro(response.card.token, response.card.transaction_reference).then(function () {
        ...
        SweetAlert("error", "Tarjeta rechazada");
        resetPaymentButton();
      ...
      if (response.error.type.includes("Card already added")) {
        ...
        // Comprobar si se encontrÃ³ el número y almacenarlo en una variable
        var token = resultado ? resultado[0] : null;

        console.log(response)
        completarRegistroEmpresaConCobro(token, null).then(function () {
          submitButton.style.display = 'none';
          $("#modal-pago").modal('hide');
          resetCheckoutState({ keepDraftHandled: true });
          swal({
...
function completarRegistroEmpresaConCobro(token, transaction_reference = null) {
