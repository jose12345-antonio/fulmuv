...
const empresaImagenFallback = "../img/FULMUV-LOGO-60X60.png";
...
function setDashboardCount(key, value) {
  dashboardCounts[key] = Number(value || 0);
  dashboardLoaded[key] = true;
  updateDashboardUI();
}

function updateDashboardUI() {
  $("#count_productos").text(dashboardCounts.productos);
  $("#count_servicios").text(dashboardCounts.servicios);
  $("#count_vehiculos").text(dashboardCounts.vehiculos);
  $("#count_ordenes").text(dashboardCounts.ordenes);
  $("#count_eventos").text(dashboardCounts.eventos);
  $("#count_empleos").text(dashboardCounts.empleos);

  const publicaciones = dashboardCounts.productos +
    dashboardCounts.servicios +
    dashboardCounts.vehiculos +
    dashboardCounts.eventos +
    dashboardCounts.empleos;

  $("#count_publicaciones").text(publicaciones);
  $("#count_ordenes_resumen").text(dashboardCounts.ordenes);

  const allReady = Object.values(dashboardLoaded).every(Boolean);
  if (allReady) {
    $("#dashboard_updated_at").text(new Date().toLocaleString());
    renderDashboardChart();
  }
}

function renderDashboardChart() {
  const el = document.getElementById("empresaDashboardChart");
  if (!el || !window.echarts) return;

  if (!dashboardChart) {
    dashboardChart = window.echarts.init(el);
  }

  const labels = ["Productos", "Servicios", "Vehículos", "Órdenes", "Eventos", "Empleos"];
  const values = [
    dashboardCounts.productos,
    dashboardCounts.servicios,
    dashboardCounts.vehiculos,
    dashboardCounts.ordenes,
    dashboardCounts.eventos,
    dashboardCounts.empleos
  ];
...
function normalizeMembresiaNombre(nombre) {
  return (nombre || "").toString().trim().toLowerCase();
}

function applyMembresiaVisibility(nombreMembresia) {
  const memb = normalizeMembresiaNombre(nombreMembresia);
  ...
  // Defaults: mostrar todo
  showSections(["productos", "servicios", "vehiculos", "ordenes", "eventos", "empleos", "chart", "summary"]);

  if (memb.includes("basicmuv")) {
    // Solo servicios
    hideSections(["productos", "vehiculos", "ordenes", "eventos", "empleos", "chart", "summary"]);
  } else if (memb.includes("onemuv")) {
    // Oculta empleos (galería no existe en esta vista)
    hideSections(["empleos"]);
  }
}

function cargarDashboardResumen() {
  const empresaId = id_empresa || $("#id_empresa").val();
  const tipo = tipo_user || "empresa";

  if (!empresaId) {
    return;
  }

  const renderLista = (selector, items, getTitle, getDate) => {
    const $wrap = $(selector);
    if (!$wrap.length) return;
    if (!items.length) {
      $wrap.html(`<span class="text-600 fs-10">Sin registros</span>`);
      return;
    }
    $wrap.html(items.map(item => {
      const titulo = getTitle(item) || "Sin título";
      const fecha = getDate(item);
      return `
        <div class="list-item d-flex justify-content-between align-items-center">
          <span class="text-700 fs-10 text-truncate title">${titulo}</span>
          ${fecha ? `<span class="badge-date">${fecha}</span>` : ""}
        </div>
      `;
    }).join(""));
  };

  $.post('../api/v1/fulmuv/productos/idEmpresa', { id_empresa: empresaId }, function (res) {
    const data = typeof res === "string" ? JSON.parse(res) : res;
    const productos = Array.isArray(data?.data) ? data.data : [];
    setDashboardCount("productos", productos.length);
    ...
    renderLista(
      "#list_recientes_productos",
      recientes,
      (item) => item.titulo_producto || item.nombre,
      (item) => item.created_at ? new Date(item.created_at).toLocaleDateString() : ""
    );
  }).fail(() => {
    setDashboardCount("productos", 0);
    renderLista("#list_recientes_productos", [], () => "", () => "");
  });

  $.get(`../api/v1/fulmuv/servicios/all/${empresaId}/${tipo}`, function (res) {
    const data = typeof res === "string" ? JSON.parse(res) : res;
    const servicios = Array.isArray(data?.data) ? data.data : [];
    setDashboardCount("servicios", servicios.length);
    ...
    renderLista(
      "#list_recientes_servicios",
      recientes,
      (item) => item.titulo_producto || item.nombre,
      (item) => item.created_at ? new Date(item.created_at).toLocaleDateString() : ""
    );
  }, 'json').fail(() => {
    setDashboardCount("servicios", 0);
    renderLista("#list_recientes_servicios", [], () => "", () => "");
  });

  $.get(`../api/v1/fulmuv/vehiculos/all/${empresaId}`, function (res) {
    const data = typeof res === "string" ? JSON.parse(res) : res;
    const vehiculos = Array.isArray(data?.data) ? data.data : [];
    setDashboardCount("vehiculos", vehiculos.length);

    const recientes = vehiculos
      .slice()
      .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0))
      .slice(0, 5);

    renderLista(
      "#list_recientes_vehiculos",
      recientes,
      (item) => item.titulo_producto || item.nombre || item.modelo,
      (item) => item.created_at ? new Date(item.created_at).toLocaleDateString() : ""
    );
  }, 'json').fail(() => {
    setDashboardCount("vehiculos", 0);
    renderLista("#list_recientes_vehiculos", [], () => "", () => "");
  });

  $.get(`../api/v1/fulmuv/eventosEmpresa/${empresaId}/${tipo}`, function (res) {
    const data = typeof res === "string" ? JSON.parse(res) : res;
    const eventos = Array.isArray(data?.data) ? data.data : [];
    setDashboardCount("eventos", eventos.length);
    ...
    renderLista(
      "#list_recientes_eventos",
      recientes,
      (item) => item.titulo,
      (item) => item.created_at ? new Date(item.created_at).toLocaleDateString() : ""
    );
  }, 'json').fail(() => {
    setDashboardCount("eventos", 0);
    renderLista("#list_recientes_eventos", [], () => "", () => "");
  });

  $.get(`../api/v1/fulmuv/empleos/all/${empresaId}/${tipo}`, function (res) {
    const data = typeof res === "string" ? JSON.parse(res) : res;
    const empleos = Array.isArray(data?.data) ? data.data : [];
    setDashboardCount("empleos", empleos.length);

    const recientes = empleos
      .slice()
      .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0))
      .slice(0, 5);

    renderLista(
      "#list_recientes_empleos",
      recientes,
      (item) => item.titulo,
      (item) => item.created_at ? new Date(item.created_at).toLocaleDateString() : ""
    );
  }, 'json').fail(() => {
    setDashboardCount("empleos", 0);
    renderLista("#list_recientes_empleos", [], () => "", () => "");
  });
}
...
$(document).ready(function () {
  ...
  $.get('../api/v1/fulmuv/empresas2/' + id_empresa + '/' + tipo_user + '/detalle', {}, function (returnedData) {
    ...
    if (returned.error == false) {
      ...
      const imagenEmpresa = tipo_user == "sucursal" ? returned.data.imagen : returned.data.img_path;
      $("#imagen_empresa")
        .attr("src", imagenEmpresa || empresaImagenFallback)
        .attr("onerror", "this.onerror=null;this.src='../img/FULMUV-LOGO-60X60.png';");
      ...
      applyMembresiaVisibility(nombreMem);
      ...
      const verificadoAprobado = ver && Number(ver.verificado) === 1;
      const verificacionRechazada = ver && Number(ver.rechazo_verificacion_empresa) === 1;
      const isVerified = Boolean(verificadoAprobado && !verificacionRechazada);
  ...
  $.post('../api/v1/fulmuv/ordenes/', {
    ...
    id_empresa: $("#id_empresa").val() || id_empresa
  }, function (returnedData) {
    var returned = JSON.parse(returnedData)
    if (returned.error == false) {
      setDashboardCount("ordenes", Array.isArray(returned.data) ? returned.data.length : 0);
