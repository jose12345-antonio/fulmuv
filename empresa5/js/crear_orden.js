              ...
              <div class="overflow-hidden">
                ...
                <div class="d-flex flex-between-center px-3 mb-1 align-items-start">
                  ...
                  <div class="d-flex">
                    ...
                    <a onclick="agregarAlCarrito(${id_catalogo_global},${producto.id_producto})" class="btn btn-sm btn-falcon-default" data-bs-toggle="tooltip" data-bs-placement="top" title="Agregar al carrito">
...
function agregarAlCarrito(idCatalogo, productId) {
  console.log(productId)
  let carritoActual = carrito.find(item => item.id_catalogo === idCatalogo);

  if (!carritoActual) {
    // Si no existe un carrito para este catálogo, lo creamos
    carritoActual = { id_catalogo: idCatalogo, productos: [] };
    carrito.push(carritoActual);
  }
  console.log(carrito)
  console.log(carritoActual)
  // Buscar si el producto ya está en el carrito
  const productoEnCarrito = carritoActual.productos.find(item => item.id === productId);
  // Si no está, lo agregamos con cantidad 1
  toastr.options.timeOut = 1500; // Configuración temporal para esta notificación
  if (!productoEnCarrito) {
    carritoActual.productos.push({ id: productId, cantidad: 1 });
    toastr.success("Producto agregado!");
  } else {
    toastr.warning("El producto ya existe en el carrito!");
  }
  $(".carrito_items").text(carrito[0] ? carrito[0].productos.length : 0)
...
function getCatalogo(id_sucursal) {
  ...
  $.get('../api/v1/fulmuv/sucursales/' + id_sucursal + '/catalogo', {}, function (returnedData) {
    ...
    $("#lista_productos").empty()
    if (returned.error == false) {
      id_catalogo_global = returned.data.id_catalogo;
