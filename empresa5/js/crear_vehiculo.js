...
function getReferenciasPermitidas() {
  return ['UTV', 'UTVS', 'ATV', 'ATVS', 'CAMIONES Y PESADOS', 'CAMIONES', 'PESADOS'].map(normalizarTextoVehiculo);
}
...
function sanitizarReferenciaVisible(ref) {
  ...
  if (['UTV', 'UTVS', 'ATV', 'ATVS'].includes(normalized)) return '';
...
$(document).ready(function () {
  ...
  $.get('../api/v1/fulmuv/tiposAuto/', {}, function (r) {
    const d = parseAjaxJSON(r);
    if (!d.error) {
      const tiposBloqueados = new Set(['UTV', 'UTVS', 'ATV', 'ATVS', 'CAMIONES Y PESADOS', 'CAMIONES', 'PESADOS'].map(normalizarTextoVehiculo));
...
function cargarVehiculoParaEditar(id){
  $.ajax({
    ...
    success: function(r){
      ...
      // ✅ Cargar Referencia -> Marca -> Modelos
...
function asignarModelo(){
  ...
  $.get('../api/v1/fulmuv/getModeloById/' + id_modelo, {}, function (returnedData) {
    ...
    const referenciaModelo = String(d.referencia || '').split(',')[0]?.trim();
    if (referenciaModelo) {
      setSelect2ValueByText($("#referencia"), referenciaModelo);
    }
  });
}
