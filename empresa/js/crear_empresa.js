...
function collectCurrentFormData() {
  ...
  const payload = {
    ...
    correo_facturacion: $('#correo_facturacion').length ? $('#correo_facturacion').val() : (empresaRegistroPayload?.correo_facturacion || ''),
...
function buildEmpresaCreateErrorMessage(returned, fallbackMessage) {
  const parts = [];
  const msg = String(returned?.msg || fallbackMessage || '').trim();
  const sqlError = String(returned?.sql_error || '').trim();
  const sql = String(returned?.sql || '').trim();
  const facturaMsg = String(returned?.factura?.msg || returned?.datos?.factura?.msg || '').trim();
  const facturaSource = String(returned?.factura?.source || returned?.datos?.factura?.source || '').trim();

  if (msg) parts.push(msg);
  if (facturaMsg && facturaMsg !== msg) parts.push(`Factura: ${facturaMsg}`);
  if (facturaSource) parts.push(`Origen factura: ${facturaSource}`);
  if (sqlError) parts.push(`SQL: ${sqlError}`);
  if (sql) parts.push(`Consulta: ${sql}`);

  return parts.join('\n');
}

function buildEmpresaSuccessMessage(returned, fallbackMessage) {
  const facturaInfo = returned?.factura || returned?.datos?.factura || null;
  if (facturaInfo && facturaInfo.emitida === false) {
    return `${fallbackMessage}\n\nNota de facturacion: ${facturaInfo.msg || 'La factura quedó pendiente de revisión.'}`;
  }
  return fallbackMessage;
}
...
function promptResumeCheckoutDraft() {
  // Draft resume desactivado — se limpia al cerrar el modal
  clearCheckoutDraft();
  draftResumeHandled = true;
}

// Limpiar draft y estado al cerrar cualquiera de los dos modales (X, Cancelar o clic fuera)
// No limpiar si es una transición interna hacia el modal de pago
$(document).on('hidden.bs.modal', '#staticBackdrop, #modal-pago', function () {
  if (transitandoAPago) return;
  clearCheckoutDraft();
...
function renderMembresiasSelect() {
  ...
  // FULMUV en el centro (índice 1), los demás planes a los lados
  const allKeys = Object.keys(groupedMembresias);
  const planOrder = ['onemuv', 'fulmuv', 'basicmuv'];
  const sortedKeys = [
    ...planOrder
      .map(planKey => allKeys.find(k => getPlanKey(k) === planKey))
      .filter(Boolean),
    ...allKeys.filter(k => !planOrder.includes(getPlanKey(k)))
  ];
  ...
  sortedKeys.forEach(key => {
    ...
    const cardHtml = `
      <div class="col-md-4 col-sm-12 mb-4 d-flex${isFulMuv ? ' membresia-col-fulmuv' : ''}" data-plan-tab="${getPlanKey(nombre)}" data-plan-label="${nombre}">
        <div class="plan-card${isFulMuv ? ' plan-card--popular' : ''} flex-fill">

          ${isFulMuv ? `<div class="plan-card__popular-bar">&#9733; Más Popular</div>` : ''}

          <div class="plan-card__body">

            <div class="plan-card__name">${nombre}</div>

            <div class="plan-card__selector">
              <select id="${selectId}" data-group="${key}" class="form-select form-select-sm w-auto mx-auto">
                ${options}
              </select>
            </div>

            ${isFulMuv ? `
              <div id="${sucursalWrapId}" class="suc-toggle-wrap" style="${String(defaultPlan.dias_permitidos) === '365' ? '' : 'display:none!important;'}">
                <input class="suc-toggle-input" type="checkbox" id="${sucursalCheckId}">
                <label class="suc-toggle-label" for="${sucursalCheckId}">
                  <span class="suc-toggle-check"><span class="fas fa-check suc-toggle-check-icon"></span></span>
                  <span class="suc-toggle-icon"><span class="fas fa-store"></span></span>
                  <span class="suc-toggle-title">Tengo sucursales</span>
                </label>
              </div>
            ` : ``}
            ...
            <div class="plan-card__price-wrap">
              <div class="plan-card__price">
                <sup class="plan-card__currency">$</sup>
                <span id="${precioId}" class="plan-card__amount" data-base="${defaultPlan.costo}">${defaultPlan.costo}</span>
              </div>
              <div class="plan-card__period" id="${periodoId}">/ ${diasToText(defaultPlan.dias_permitidos)}</div>
            </div>

            <div class="text-center" id="${badgeContainerId}">${badgeInicial}</div>
            <div class="text-center" id="${promoDetailId}"></div>

            <button id="${btnId}" class="plan-card__btn${isFulMuv ? ' plan-card__btn--popular' : ''}" type="button">
              Comprar
            </button>

            <div class="plan-card__features-title">Características:</div>
            <ul class="plan-card__features list-unstyled mb-0">
              ${parts.preview}
            </ul>
            ${parts.hasMore ? `
              ...
              <div class="text-center mt-2">
                <button id="${toggleId}" class="btn btn-link p-0 text-decoration-none" type="button"
  ...
  initMembresiaCarousel(sortedKeys.findIndex(k => getPlanKey(k) === 'fulmuv'));
}

function initMembresiaCarousel(startIdx) {
  startIdx = Number(startIdx);
  if (!Number.isFinite(startIdx) || startIdx < 0) startIdx = 0;

  const track = $('#contenedor-membresias');
  const tabsEl = $('#membresiaTabNav');
  if (!track.length || !tabsEl.length) return;

  const cards = track.children('[data-plan-tab]');
  ...
  cards.each(function (idx) {
    const planKey = String($(this).data('plan-tab') || '');
    const rawLabel = String($(this).data('plan-label') || '');
    const label = labelMap[planKey] || rawLabel || `Plan ${idx + 1}`;
    tabsEl.append(`
      <button type="button" class="membresia-tab-btn${planKey === 'fulmuv' ? ' is-fulmuv' : ''}" data-idx="${idx}">
        ${label}
      </button>
    `);
  });
  ...
  function goTo(idx) {
    ...
    cards.removeClass('membresia-tab-active').eq(current).addClass('membresia-tab-active');
    tabsEl.find('.membresia-tab-btn')
      .removeClass('is-active')
      .attr('aria-selected', 'false')
      .filter(`[data-idx="${current}"]`)
      .addClass('is-active')
      .attr('aria-selected', 'true');
  }

  tabsEl.off('click.tabs').on('click.tabs', '.membresia-tab-btn', function () {
    goTo(Number($(this).data('idx')));
  });
...
function diasToText(dias) {
  if (String(dias) === "30") return "mensual (30)";
  if (String(dias) === "180") return "semestral (180)";
  return "anual (365)";
}
...
function evaluatePromotionForContext(context, agente = agenteAplicado, codigo = codigoAplicado) {
  ...
  if (tipo === 'general' || tipo === 'general_anual') {
    ...
    result.detailHtml = `
      ...
      <div class="small text-700">Precio sin código: ${formatCurrencyValue(regularPrice)}</div>
      <div class="small text-primary fw-semi-bold">Con tu código pagas: ${formatCurrencyValue(promoPrice)}</div>
    `;
    result.promotionMessage = `Precio sin código: ${formatCurrencyValue(regularPrice)}. Con tu código pagas: ${formatCurrencyValue(promoPrice)}.`;
    return result;
  }

  if (tipo === 'fulmuv') {
    const isCode12 = codigo === '12FULMUV777';
    const isCode6  = codigo === '6FULMUV777';

    if (isCode12) {
      if (!isFulmuvPlan || !isAnnual) return result;
    } else if (isCode6) {
      if (!isFulmuvPlan || !isSemiAnnual) return result;
    } else {
    ...
    const periodoTexto = isAnnual ? 'anual' : 'semestral';
    result.detailHtml = `
      <div class="small text-success fw-semi-bold mt-2">Código ${codigo} aplicado.</div>
      <div class="small text-primary fw-semi-bold">Cobro de hoy: ${formatCurrencyValue(1)}</div>
      <div class="small text-700">Desde el mes 2: ${formatCurrencyValue(nextMonthAmount)} / ${periodoTexto}</div>
      <div class="small text-muted">Renovación automática: ${formatCurrencyValue(regularPrice)} / ${periodoTexto} (precio habitual)</div>
    `;
    result.promotionMessage = `Cobro de hoy: ${formatCurrencyValue(1)}. Desde el mes 2: ${formatCurrencyValue(nextMonthAmount)} / ${periodoTexto}. Renovación: ${formatCurrencyValue(regularPrice)} / ${periodoTexto}.`;
    result.detailLabel = 'Cobro de hoy';
    return result;
  }
...
function buildPlanDetailBlock(promo, state) {
  if (!state) return '';

  const precioNormal   = promo.precio_original || state.regularPrice || 0;
  const diasStr        = String(state.dias || '');
  const periodoMeses   = diasStr === '180' ? 6 : diasStr === '365' ? 12 : null;
  const periodoLabel   = diasStr === '180' ? 'semestral'
                       : diasStr === '365' ? 'anual'
                       : (state.periodicidad || '');
  const conSucursal    = state.conSucursal ? ' + sucursales' : '';
  const planFullLabel  = `${state.nombre || '-'} ${periodoLabel}${conSucursal}`;

  const isFulmuvPromo  = promo.aplica_promocion && promo.tipo === 'fulmuv';
  const isGenPromo     = promo.aplica_promocion && !isFulmuvPromo;

  // Fila: código aplicado
  const codeRow = promo.aplica_promocion
    ? `<div class="plan-detail-row is-separator d-none">
         <span class="plan-detail-lbl"><span class="fas fa-tag me-1"></span>Código</span>
         <span class="plan-detail-val"><span class="plan-detail-code-badge"><span class="fas fa-check me-1"></span>${promo.codigo || ''}</span></span>
       </div>`
    : '';

  if (isFulmuvPromo) {
    // Promo especial: cobro $1 hoy, desde el mes 2 precio del periodo
    return `
      <div class="plan-detail-grid">
        <div class="plan-detail-row">
          <span class="plan-detail-lbl">Plan</span>
          <span class="plan-detail-val">${planFullLabel}</span>
        </div>
        <div class="plan-detail-row is-separator">
          <span class="plan-detail-lbl">Precio normal del plan</span>
          <span class="plan-detail-val is-muted">${formatCurrencyValue(precioNormal)}</span>
        </div>
        ${codeRow}
        <div class="plan-detail-row is-today">
          <span class="plan-detail-lbl"><strong>Cobro de hoy</strong></span>
          <span class="plan-detail-val is-primary" style="font-size:18px;">${formatCurrencyValue(promo.monto_hoy || 1)}</span>
        </div>
        <div class="plan-detail-row is-highlight">
          <span class="plan-detail-lbl">Desde el mes 2</span>
          <span class="plan-detail-val is-primary">${formatCurrencyValue(promo.monto_siguiente_mes)} / ${periodoLabel}</span>
        </div>
        <div class="plan-detail-row">
          <span class="plan-detail-lbl">Renovación automática</span>
          <span class="plan-detail-val is-muted">${formatCurrencyValue(precioNormal)} / ${periodoLabel} <span style="font-size:11px;color:#94a3b8;">(precio habitual)</span></span>
        </div>
      </div>`;
  }

  if (isGenPromo) {
    // Promo general: descuento en precio
    const precioPromo = promo.precio_promocional || precioNormal;
    return `
      <div class="plan-detail-grid">
        <div class="plan-detail-row">
          <span class="plan-detail-lbl">Plan</span>
        ...
        <div class="plan-detail-row is-separator">
          <span class="plan-detail-lbl">Precio sin código</span>
          <span class="plan-detail-val is-muted">${formatCurrencyValue(precioNormal)}</span>
        </div>
        ${codeRow}
        <div class="plan-detail-row is-today">
          <span class="plan-detail-lbl"><strong>Con tu código pagas</strong></span>
          <span class="plan-detail-val is-promo" style="font-size:18px;">${formatCurrencyValue(precioPromo)}</span>
        </div>
        ${periodoMeses ? `<div class="plan-detail-row">
          <span class="plan-detail-lbl">Renovación automática (${periodoLabel})</span>
          <span class="plan-detail-val is-muted">${formatCurrencyValue(precioNormal)} / ${periodoLabel}</span>
        </div>` : ''}
      </div>`;
  }
  ...
  return `
    <div class="plan-detail-grid">
      ...
      <div class="plan-detail-row is-today">
        <span class="plan-detail-lbl"><strong>Cobro de hoy</strong></span>
        <span class="plan-detail-val is-primary" style="font-size:18px;">${formatCurrencyValue(promo.monto_hoy || precioNormal)}</span>
      </div>
      ${periodoMeses ? `<div class="plan-detail-row">
        <span class="plan-detail-lbl">Renovación automática (${periodoLabel})</span>
        <span class="plan-detail-val is-muted">${formatCurrencyValue(precioNormal)} / ${periodoLabel}</span>
  ...
  container.html(`
    ...
    <div class="payment-summary-columns">
      ...
      <div class="payment-summary-panel">
        ...
        <!-- Sub-sección: Usuario -->
        <div class="psp-subsection">
          <div class="psp-subsection-header">
            <span class="fas fa-user psp-subsection-icon"></span>
            <span class="psp-subsection-title">Usuario</span>
          </div>
          <div class="payment-summary-grid">
            <div class="payment-summary-item is-wide">
        ...
        <!-- Sub-sección: Facturación -->
        <div class="psp-subsection">
          <div class="psp-subsection-header">
            <span class="fas fa-file-invoice psp-subsection-icon"></span>
            <span class="psp-subsection-title">Facturación</span>
          </div>
          <div class="payment-summary-grid">
            <div class="payment-summary-item">
            ...
            <div class="payment-summary-item is-wide">
              <span class="payment-summary-label">Correo de facturacion</span>
              
              <div class="payment-summary-value">${empresa.correo_facturacion || '-'}</div>
            </div>
          </div>
        </div>
        ...
        <div class="psp-subsection">
          <div class="psp-subsection-header">
            <span class="fas fa-credit-card psp-subsection-icon"></span>
            <span class="psp-subsection-title">Pago</span>
          </div>
          ${buildPlanDetailBlock(promo, checkoutSelectionState)}
        </div>
      </div>
    </div>
  `);
...
function getEmpresaWizardSummaryData() {
  return {
    ...
    correo_facturacion: $('#correo_facturacion').val() || empresaRegistroPayload?.correo_facturacion || '-',
...
function renderEmpresaWizardSummary() {
  ...
  $summary.html(`
    <div class="empresa-wizard-summary-grid">
      ...
      <div class="empresa-wizard-summary-item">
        <span class="empresa-wizard-summary-label">Correo de facturacion</span>
        <div class="empresa-wizard-summary-value">${d.correo_facturacion}</div>
      </div>
      <div class="empresa-wizard-summary-item">
...
function validateEmpresaWizardStep(step) {
  ...
  if (current === 3) {
    if (!$('#correo_facturacion').val().trim()) return showEmpresaWizardFieldError('#correo_facturacion', 'Debes completar el correo de facturacion.');
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test($('#correo_facturacion').val().trim())) return showEmpresaWizardFieldError('#correo_facturacion', 'Debes ingresar un correo de facturacion valido.');
          ...
          <div class="empresa-wizard-body">
            ...
            <div class="empresa-wizard-section" data-step="3">
              ...
              <div class="row g-3">
                ...
                <div class="col-12 col-lg-6"><label class="form-label">Correo de facturacion</label><input class="form-control" type="email" id="correo_facturacion" placeholder="Correo de facturacion"></div>
                ...
                <div class="col-12">
                  ...
                  <label class="form-check-label" for="chkLegales">
                    ...
                    <a href="../documentos/3_1_T%C3%A9rminos_y_Condiciones_de_Uso_Pol%C3%ADtica_de_Privacidad_Aviso_Legal_y_Descargos_de_Responsabilidad_para_Proveedores_de_FULMUV.pdf" target="_blank" rel="noopener" class="link-primary fw-bold">Términos y Condiciones para Proveedores</a>,
                    la <a href="../documentos/3_1_Pol%C3%ADtica_de_Privacidad_y_Cookies_de_FULMUV.pdf" target="_blank" rel="noopener" class="link-primary fw-bold">Política de Privacidad y Cookies</a>
                    y el <a href="../documentos/3_1_T%C3%A9rminos_y_Condiciones_de_Uso_Pol%C3%ADtica_de_Privacidad_Aviso_Legal_y_Descargos_de_Responsabilidad_para_Proveedores_de_FULMUV.pdf" target="_blank" rel="noopener" class="link-primary fw-bold">Aviso Legal</a> de FULMUV.
                  </label>
                  </div>
                </div>
                <div class="col-12">
                  ...
                  <label class="form-check-label" for="chkEnvios">
                    Acepto las <a href="../documentos/3_2_NW_Te%CC%81rminos_y_Condiciones_de_Envi%CC%81os_y_Logi%CC%81stica_de_FULMUV.pdf" target="_blank" rel="noopener" class="link-primary fw-bold">Condiciones de Uso del Servicio de Envíos y Logística FULMUV – GRUPO ENTREGAS</a> y comprendo mis responsabilidades sobre embalaje, valor comercial y respaldo de envío.
                  </label>
                </div>
                </div>
                <div class="col-12">
...
async function saveEmpresa() {
  ...
  if ((!esOneMuv && nombre == "") || nombre_titular == "" || !username || !email || !password || !repeat_password || password !== repeat_password || provincia == "" || canton == "" || razon_social == "" || celular == "" || correo_facturacion == "" || cedula_ruc == "" || !$('#chkLegales').is(':checked') || !$('#chkEnvios').is(':checked')) {
    return;
  } else {
    try {
...
function crearEmpresaAntesDePago() {
  ...
  return new Promise((resolve, reject) => {
    $.post('../api/v1/fulmuv/empresas/create', {
      ...
      telefono_facturacion: empresaRegistroPayload.celular,
      correo_facturacion: empresaRegistroPayload.correo_facturacion,
      ...
      reject(new Error(buildEmpresaCreateErrorMessage(returned, 'No se pudo crear la empresa antes del pago.')));
    });
  });
}
...
function tokenizarTarjeta(email) {
  ...
  let environment = 'stg';
  let application_code = 'TESTNUVEISTG-EC-CLIENT';
  let application_key = '0zf8cUS9UQP2TVXNW1GvXYFXfRMLjH';
  ...
  const getResultAlertMeta = (response) => {
  ...
  const extractCardTokenCrear = (response) => {
    const candidates = [
      response?.card?.token,
      response?.card?.card?.token,
    ...
    const matchToken = errorType.match(/Card already added[^:]*:\s*(\S+)/i);
    return matchToken ? String(matchToken[1] || '').trim() : '';
  };

  const extractTransactionReferenceCrear = (response) => {
    const candidates = [
    ...
    for (let i = 0; i < candidates.length; i += 1) {
      const reference = String(candidates[i] || '').trim();
  ...
  const extractHolderNameCrear = (response) => {
    const candidates = [
  ...
  let responseCallback = response => {
    const usableToken = extractCardTokenCrear(response);
    const transactionReference = extractTransactionReferenceCrear(response);
    const cardMeta = {
      ultimos_digitos: String(response?.card?.number || response?.transaction?.card?.number || '').replace(/\D/g, '').slice(-4) || null,
      marca: _normalizeBrandCrear(response?.card?.type || response?.transaction?.card?.type || ''),
      exp_year: response?.card?.expiry_year || response?.transaction?.card?.expiry_year || null,
      exp_month: response?.card?.expiry_month || response?.transaction?.card?.expiry_month || null,
      holder_name: extractHolderNameCrear(response),
    };

    if (response.card) {

      // registrar el token en la bd
      if (String(response.card.status || '').toLowerCase() == "valid" && usableToken) {
        console.log(response)

        completarRegistroEmpresaConCobro(usableToken, transactionReference, cardMeta).then(function (returned) {
          submitButton.style.display = 'none';
          $("#modal-pago").modal('hide');
          resetCheckoutState({ keepDraftHandled: true });
          swal({
            title: "!Pago registrado con éxito!",
            text: buildEmpresaSuccessMessage(returned, "Haz clic en OK para acceder al sistema. \n  Bienvenido a FULMUV"),
            icon: "success",
            button: "OK",
          }, function () {
        ...
        const alertMeta = getResultAlertMeta(response);
        const rejectionDetail = getGatewayErrorMessage(response);
        console.error('Nuvei card rejected', response);
        document.getElementById('tokenize_response').innerHTML = `<div class="text-${alertMeta.type === 'warning' ? 'warning' : 'danger'} small">${(alertMeta.type === 'warning' ? alertMeta.text : rejectionDetail).replace(/\n/g, '<br>')}</div>`;
        SweetAlert(alertMeta.type === 'warning' ? "warning" : "error", alertMeta.type === 'warning' ? alertMeta.text : rejectionDetail);
        resetPaymentButton();
      ...
      if (response.error.type.includes("Card already added")) {
        ...
        completarRegistroEmpresaConCobro(usableToken, transactionReference || null, cardMeta).then(function (returned) {
          submitButton.style.display = 'none';
          $("#modal-pago").modal('hide');
          resetCheckoutState({ keepDraftHandled: true });
          swal({
  ...
  (function () {
    const _container = document.getElementById('tokenize_example');
    ...
    _obs.observe(_container, { childList: true, subtree: true });
  })();
...
function completarRegistroEmpresaConCobro(token, transaction_reference = null, cardMeta = {}) {
  ...
  const tipo_code = __mapTipoPagoCode(tipoSeleccionado);
  const meses_send = __normalizarMeses(tipoSeleccionado, mesesSeleccionados);
  const montoPrueba = Number(valor_pagado) || Number(costo_seleccionado) || 0;
  ...
  const cardMetaPayload = {
    ...
    holder_name: String(cardMeta.holder_name || '').trim(),
    es_default: String(cardMeta.es_default || '').trim() || 'Y',
    gateway_uid: String(cardMeta.gateway_uid || '').trim() || gatewayUidDefault
  };

  console.log("ESTE ES LA INFO DE LA EMPRESA")
  console.log(empresaRegistroPayload)
  ...
  return new Promise((resolve, reject) => {
    ...
    $.post('../api/v1/fulmuv/empresas/create', {
      ...
      holder_name:     cardMetaPayload.holder_name,
      ...
      reject(new Error(buildEmpresaCreateErrorMessage(returned, 'No se pudo completar el registro de la empresa.')));
    }, 'json');
  });
}
...
function comprarDirecto(id_empresa, transaction_id, authorization_code, recurrente, payment_date) {
  ...
  $.post("../api/v1/fulmuv/empresas/membresiasUpdate", {
    ...
    if (!returnedData.error) {
      resetCheckoutState({ keepDraftHandled: true });
      const facturaInfo = returnedData?.datos?.factura || null;
      const successText = facturaInfo && facturaInfo.emitida === false
        ? `Haz clic en OK para acceder al sistema. \n  Bienvenido a FULMUV\n\nNota de facturacion: ${facturaInfo.msg || 'La factura quedó pendiente de revisión.'}`
        : "Haz clic en OK para acceder al sistema. \n  Bienvenido a FULMUV";
      swal({
        title: "!Pago registrado con éxito!",
        text: successText,
      ...
      swal({
        ...
        text: buildEmpresaCreateErrorMessage(returnedData, 'Haz clic en OK para intentarlo nuevamente.'),
        icon: 'error',
        button: 'OK'
      });
    }
...
$(document).on('click', '#btnAplicarCodigo', function () {
  ...
  // Auto-seleccionar el periodo correcto según el código
  if (codigo === '12FULMUV777') {
    $('#select_fulmuv').val('365').trigger('change');
  } else if (codigo === '6FULMUV777') {
    $('#select_fulmuv').val('180').trigger('change');
  }
