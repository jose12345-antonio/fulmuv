  ...
  waitMapsReady(() => {
    ...
    const onShown = () => {
      ...
      const hasCoords =
        ...
        !Number.isNaN(parseFloat(latitud)) &&
        !Number.isNaN(parseFloat(longitud));
...
function getEventoCoordsPayload() {
  ...
  if (typeof latitud === "undefined" || latitud === null || Number.isNaN(parseFloat(latitud))) {
    latitud = defaultPos().lat;
  }

  if (typeof longitud === "undefined" || longitud === null || Number.isNaN(parseFloat(longitud))) {
    longitud = defaultPos().lng;
  }
...
function formatEventoHourShort(valor) {
  if (!valor) return "";
  const d = new Date(String(valor).replace(" ", "T"));
...
function formatEventoWeekRange(startValue, endValue) {
  const startHour = formatEventoHourShort(startValue);
  const endHour = formatEventoHourShort(endValue);
  if (startHour && endHour) return `${startHour} - ${endHour}`;
  return startHour || endHour || "Horario pendiente";
}
...
function renderEventosCalendar(eventos) {
  ...
  eventosCalendar = new FullCalendar.Calendar(calendarEl, {
    ...
    eventContent(arg) {
      ...
      if (isWeekView) {
        const timeRange = formatEventoWeekRange(arg.event.start, arg.event.end);
        return {
          html: `
            <div class="evento-week-chip">
              <div class="evento-week-chip-head">
                <span class="evento-week-time">${timeRange}</span>
                <span class="evento-week-status ${status.className || ""}" title="${status.label || "Evento"}">
                  <i class="${status.icon || "far fa-calendar-alt"}"></i>
                </span>
              </div>
              <strong>${arg.event.title}</strong>
              <div class="evento-week-meta">
                <span class="evento-week-type">${tipo ? tipo : "Evento"}</span>
                <span class="evento-week-label">${status.label || "Programado"}</span>
              </div>
            </div>
          `
        };
    ...
    eventDidMount(info) {
      ...
      if (info.view.type === "timeGridWeek") {
        info.el.style.background = isDarkMode
          ? `linear-gradient(180deg, ${accent} 0%, rgba(15, 23, 42, 0.96) 100%)`
          : `linear-gradient(180deg, ${accent} 0%, ${accent}dd 100%)`;
        info.el.style.border = "0";
        info.el.style.borderLeft = `4px solid ${contrastColor}`;
        info.el.style.boxShadow = isDarkMode
...
function addEventos() {
  ...
  $("#tipoEntrada, #tipoPrecio").off("change.pre").on("change.pre", function () {
    ...
    let tipoPrecio = $("#tipoPrecio").val();

    if (tipoEntrada === "gratuita") {
      tipoPrecio = "gratis";
      $("#tipoPrecio").val("gratis");
    }
...
async function guardarEventos(modo = "crear") {
  ...
  try {
    ...
    if (tipo_entrada === "pagada" && !tipo_precio) faltantes.push("Precio (tipo)");
    ...
    const coords = getEventoCoordsPayload();

    const precioPayload = {
      tipo: tipo_entrada === "gratuita" ? "gratis" : tipo_precio,
...
async function editEventos(id_eventos) {
  ...
  try {
    ...
    if (galeriaFiles.length) {
      ...
      await postJSON("../api/v1/fulmuv/eventos/galeria/create", {
        id_evento: id_eventos,
        imagenes: JSON.stringify(galeriaUrls)
      });
    }
...
function normalizeEventoMediaUrl(path) {
  const raw = String(path || "").trim();
  if (!raw) return "";
  if (/^https?:\/\//i.test(raw)) return raw;
  if (raw.startsWith("../") || raw.startsWith("./") || raw.startsWith("/")) return raw;
  return `../admin/${raw.replace(/^admin\//, "")}`;
}
...
function deleteGaleriaEvento(idGaleria) {
  return new Promise((resolve, reject) => {
    $.ajax({
      url: "../api/v1/fulmuv/eventos/galeria/delete",
      method: "POST",
      data: { id_galeria: idGaleria },
      success: (res) => resolve(typeof res === "string" ? JSON.parse(res) : res),
      error: (xhr) => reject(xhr)
    });
  });
...
function renderEventoMediaActual(evento) {
  ...
  const portada = normalizeEventoMediaUrl(evento?.imagen || evento?.portada_evento || "");
  ...
  const portadaHtml = portada
    ? `<div class="col-md-4">
         <div class="border rounded-3 p-2 h-100">
           ...
           <img src="${portada}" alt="Portada actual" class="img-fluid rounded" style="width:100%;max-height:180px;object-fit:cover;" onerror="this.closest('.border').insertAdjacentHTML('beforeend','<div class=&quot;text-muted fs-10 mt-2&quot;>No se pudo cargar la portada actual.</div>');this.remove();">
         </div>
       </div>`
    : "";
  ...
  const galeriaHtml = galeria.length
    ? galeria.map(img => `
        <div class="col-6 col-md-3">
          <div class="position-relative overflow-hidden rounded border" style="height:120px;">
            <img src="${normalizeEventoMediaUrl(img.imagen)}" alt="Galería actual" class="img-fluid w-100 h-100" style="object-fit:cover;" onerror="this.closest('.position-relative').insertAdjacentHTML('beforeend','<div class=&quot;text-muted fs-10 mt-1 p-2&quot;>Imagen no disponible</div>');this.remove();">
            <div class="position-absolute top-50 start-50 translate-middle d-flex gap-2">
              <button type="button"
                class="btn btn-sm btn-light rounded-circle shadow btn-view-galeria-actual"
                data-url="${normalizeEventoMediaUrl(img.imagen)}"
                title="Ver imagen"
                style="width:38px;height:38px;display:flex;align-items:center;justify-content:center;">
                <i class="fas fa-external-link-alt"></i>
              </button>
              <button type="button"
                class="btn btn-sm btn-danger rounded-circle shadow btn-delete-galeria-actual"
                data-id="${img.id_galeria}"
...
function prepararFormularioEventoEdicion(evento) {
  ...
  $(document)
    .off("click.eventoGaleriaView", "#eventoMediaActual .btn-view-galeria-actual")
    .on("click.eventoGaleriaView", "#eventoMediaActual .btn-view-galeria-actual", function () {
  ...
  $(document)
    .off("click.eventoGaleriaDelete", "#eventoMediaActual .btn-delete-galeria-actual")
    .on("click.eventoGaleriaDelete", "#eventoMediaActual .btn-delete-galeria-actual", async function () {
      const idGaleria = Number($(this).data("id"));
      if (!idGaleria) return;

      const confirmar = window.confirm("¿Deseas eliminar esta imagen de la galería?");
      if (!confirmar) return;

      try {
        const res = await deleteGaleriaEvento(idGaleria);
        if (res.error) {
          SweetAlert("error", res.msg || "No se pudo eliminar la imagen.");
          return;
        }

        evento.galeria = (evento.galeria || []).filter(img => Number(img.id_galeria) !== idGaleria);
        renderEventoMediaActual(evento);
      } catch (error) {
        SweetAlert("error", "Ocurrió un error al eliminar la imagen.");
      }
    });
  ...
  if (!map && $("#modalEvento").hasClass("show")) {
    initMapaEnModalCrear("modalEvento", { lat: latitud, lng: longitud });
  }
...
function initMapaDetalleEvento(mapId, lat, lng, titulo = "Ubicación del evento") {
  waitMapsReady(() => {
    ...
    const mapaDetalle = new google.maps.Map(mapEl, {
      center: pos,
      zoom: 15,
      mapTypeControl: false,
      streetViewControl: false,
    ...
    setTimeout(() => {
      google.maps.event.trigger(mapaDetalle, "resize");
      mapaDetalle.setCenter(pos);
    }, 250);
  });
}
  ...
  $.get(`../api/v1/fulmuv/eventos/${id_evento}`, function (resp) {
    if (resp && resp.error === false) {
      ...
      $.get(`../api/v1/fulmuv/eventos/galeria/${id_evento}`, function (gres) {
        ...
        const latEvento = parseFloat(e.latitud);
        const lngEvento = parseFloat(e.longitud);
        const tieneMapa = !Number.isNaN(latEvento) && !Number.isNaN(lngEvento);
        const mapaId = `mapaDetalleEvento_${id_evento}`;
        const mapaMarkup = tieneMapa
          ? `
            <div class="mt-4">
              <h6 class="text-uppercase text-muted mb-2">Ubicación</h6>
              <div id="${mapaId}" class="rounded border" style="width:100%;height:260px;"></div>
            </div>
          `
          : '';
        ...
        if (tieneMapa) {
          ...
          const onShown = () => {
            ...
            initMapaDetalleEvento(mapaId, latEvento, lngEvento, e.titulo || "Ubicación del evento");
          };

          modalEl.addEventListener("shown.bs.modal", onShown, { once: true });

          if (modalEl.classList.contains("show")) {
            setTimeout(() => initMapaDetalleEvento(mapaId, latEvento, lngEvento, e.titulo || "Ubicación del evento"), 0);
          }
        }
