...
function getSubCategoriaNombreById(id) {
  ...
  for (const categoria of (categorias || [])) {
    const subcategorias = Array.isArray(categoria.sub_categorias) ? categoria.sub_categorias : [];
    const found = subcategorias.find(sc => String(sc.id_sub_categoria) === value);
...
function normalizeSubCategoriaPairs(producto) {
  let pairs = normMulti(producto.subcategorias, 'id_sub_categoria', 'nombre');
  if (!pairs.length) {
    pairs = normMulti(producto.sub_categoria, 'id_sub_categoria', 'nombre');
  }

  return pairs.map(pair => ({
    id: pair.id,
    text: pair.text && pair.text !== pair.id
      ? pair.text
      : (getSubCategoriaNombreById(pair.id) || pair.text)
  }));
}
...
function initCatalogoReferenciaSelect($sel, options, clearOptions = true) {
  if ($sel.hasClass('select2-hidden-accessible')) $sel.select2('destroy');
  if (clearOptions) $sel.empty();
...
function cargarCatalogoPorReferencia(selector, entity, idKey, label, placeholder, selectedPairs = []) {
  const $sel = $(selector);
  const referencia = getReferenciaSeleccionada();
  initCatalogoReferenciaSelect($sel, { entity, label, placeholder });

  if (!referencia) {
    if (selectedPairs.length) setSelectMultiplePairs($sel, selectedPairs);
    return $.Deferred().resolve([]).promise();
  }

  return $.ajax({
    url: '../api/v1/fulmuv/catalogoVehiculo/referencia/' + encodeURIComponent(entity) + '/' + encodeURIComponent(referencia),
    method: 'GET',
    dataType: 'json'
  }).then(function (returned) {
    const data = (returned && returned.data) ? returned.data : [];
    $sel.empty();
    data.forEach(item => {
      $sel.append(`<option value="${item[idKey]}">${item.nombre}</option>`);
    });
    initCatalogoReferenciaSelect($sel, { entity, label, placeholder }, false);
    if (selectedPairs.length) setSelectMultiplePairs($sel, selectedPairs);
    return data;
  }, function () {
    return [];
...
function cargarCatalogosVehiculoPorReferencia(selecciones = {}) {
  return $.when(
    buscarModelosReferencia(getReferenciaSeleccionada(), selecciones.modelo || []),
    cargarCatalogoPorReferencia('#tipo_vehiculo', 'tipos_auto', 'id_tipo_auto', 'Tipo de vehiculo', 'Seleccione tipo de vehiculo', selecciones.tipo_vehiculo || []),
    cargarCatalogoPorReferencia('#marca', 'marcas', 'id_marca', 'Marca', 'Seleccione marca', selecciones.marca || []),
    cargarCatalogoPorReferencia('#traccion', 'tipo_traccion', 'id_tipo_traccion', 'Traccion', 'Seleccione traccion', selecciones.traccion || []),
    cargarCatalogoPorReferencia('#motor', 'funcionamiento_motor', 'id_funcionamiento_motor', 'Funcionamiento de motor', 'Seleccione funcionamiento de motor', selecciones.motor || [])
  );
}

function reorderSelectedOptions($sel, orderedIds) {
  if (!$sel || !$sel.length) return;
  const seen = new Set();
  orderedIds.map(String).forEach(id => {
    if (!id || seen.has(id)) return;
    seen.add(id);
    const $opt = $sel.find(`option[value="${CSS.escape(id)}"]`).first();
    if ($opt.length) $sel.append($opt);
  });
}

function normalizeSelectedOrder(currentValues, previousOrder) {
  const selected = (Array.isArray(currentValues) ? currentValues : [currentValues])
    .filter(v => v != null && v !== '')
    .map(String);
  const prev = Array.isArray(previousOrder) ? previousOrder.map(String) : [];
  const selectedSet = new Set(selected);
  const ordered = prev.filter(id => selectedSet.has(id));
  selected.forEach(id => {
...
function syncCategoriaSelectionOrder(preferredOrder = null) {
  const $categoria = $("#categoria");
  const selected = $categoria.val() || [];
  categoriaSelectionOrder = normalizeSelectedOrder(
    preferredOrder !== null ? preferredOrder : selected,
    categoriaSelectionOrder
  );
  reorderSelectedOptions($categoria, categoriaSelectionOrder);
  if (categoriaSelectionOrder.length !== selected.length || categoriaSelectionOrder.some((id, idx) => id !== String(selected[idx] || ''))) {
    $categoria.val(categoriaSelectionOrder).trigger('change.select2');
  }
}

function getCategoriaValuesInSelectionOrder() {
  syncCategoriaSelectionOrder();
  const selectedSet = new Set(($("#categoria").val() || []).map(String));
  return categoriaSelectionOrder.filter(id => selectedSet.has(String(id)));
}
...
$(document).ready(function () {
  ...
  $.get('../api/v1/fulmuv/categorias/', {
    ...
    if (returned.error == false) {
      ...
      $("#categoria").on('select2:select select2:unselect change', function () {
        syncCategoriaSelectionOrder();
      });
      llenarSubCategria();
    }
  ...
  $.get('../api/v1/fulmuv/tiposAuto/', {}, function (returnedData) {
    ...
    if (returned.error == false) {
      if (true) {
        initCatalogoReferenciaSelect($("#tipo_vehiculo"), {
          entity: 'tipos_auto',
      ...
      $("#tipo_vehiculo option[value='']").remove();
  ...
  $.get('../api/v1/fulmuv/marcas/', {}, function (returnedData) {
    ...
    if (returned.error == false) {
      if (true) {
        initCatalogoReferenciaSelect($("#marca"), {
          entity: 'marcas',
          label: 'Marca',
  ...
  $.get('../api/v1/fulmuv/tipo_tracccion/', {}, function (returnedData) {
    ...
    if (returned.error == false) {
      if (true) {
        initCatalogoReferenciaSelect($("#traccion"), {
          entity: 'tipo_traccion',
          label: 'Traccion',
          placeholder: 'Seleccione traccion'
        });
        return;
      }
  ...
  $.get('../api/v1/fulmuv/getFuncionamientoMotor/', {}, function (returnedData) {
    ...
    if (returned.error == false) {
      if (true) {
        initCatalogoReferenciaSelect($("#motor"), {
  ...
  if (id_producto != "") {
    $.get(`../api/v1/fulmuv/productos/${id_producto}`, function (respRaw) {
      ...
      // === Categoría (multi)
      {
        ...
        syncCategoriaSelectionOrder(catPairs.map(cat => cat.id));

        llenarSubCategria();
        setTimeout(() => {
          let subPairs = normalizeSubCategoriaPairs(p);
      ...
      {
        const modelosPairsRef = normMulti(p.modelo, 'id_modelos_autos', 'nombre');
        let tvPairsRef = normMulti(p.tipo_vehiculo, 'id_tipo_auto', 'nombre');
        if (!tvPairsRef.length) tvPairsRef = normMulti(p.tipo_auto, 'id', 'nombre');
        const marPairsRef = normMulti(p.marca, 'id', 'nombre');
        const traccionPairsRef = normMulti(p.tipo_traccion)
          .map(x => ({ id: String(x.id).trim(), text: String(x.text || x.id).trim() }))
          .filter(x => x.id && x.id !== "0" && x.id !== "null" && x.id !== "undefined");
        let motPairsRef = normMulti(p.motor, 'id_funcionamiento_motor', 'nombre');
        if (!motPairsRef.length) motPairsRef = normMulti(p.funcionamiento_motor, 'id', 'nombre');

        cargarCatalogosVehiculoPorReferencia({
          modelo: modelosPairsRef,
          tipo_vehiculo: tvPairsRef,
          marca: marPairsRef,
          traccion: traccionPairsRef,
          motor: motPairsRef
        });
      }
...
$("#referencia").on("change", async function () {
  $("#modelo, #tipo_vehiculo, #marca, #traccion, #motor").val(null).trigger("change");
  await cargarCatalogosVehiculoPorReferencia();
});
...
function llenarSubCategria() {
  ...
  const selectedBefore = ($sele.val() || []).map(String);
  ...
  const categoriaIds = (Array.isArray(sel) ? sel : [sel])
  ...
  const cats = (categorias || []).filter(c => categoriaIds.includes(parseInt(c.id_categoria, 10)));
  const subcategorias = [];
  const seenSubcategorias = new Set();

  cats.forEach(cat => {
    const items = Array.isArray(cat.sub_categorias) ? cat.sub_categorias : [];
    items.forEach(sc => {
      const id = String(sc.id_sub_categoria || '');
      if (!id || seenSubcategorias.has(id)) return;
      seenSubcategorias.add(id);
      subcategorias.push(sc);
    });
  });
  ...
  if (selectedBefore.length) {
    const available = new Set($sele.find('option').map(function () { return String(this.value); }).get());
    const stillAvailable = selectedBefore.filter(id => available.has(id));
    if (stillAvailable.length) $sele.val(stillAvailable);
  }
  $sele.trigger("change");

  $.get('../api/v1/fulmuv/atributosCategoriaCompleto/' + categoriaIds[0], {}, function (_returnedData) {
...
function addProducto() {
  ...
  var categoria = getCategoriaValuesInSelectionOrder();
