...
class DbHandler
{
    ...
    {
        $response = array();
        $stmt = $this->conn->prepare("SELECT p.id_permisos, p.permiso, p.valor, p.levels, p.created_at, p.estado, p.id_rol FROM usuarios u, rol r, permisos p WHERE u.id_usuario = ? AND u.rol_id = r.id_rol AND p.id_rol =r.id_rol;");
...
");
    ...
    public function createEmpresaExtendida(
        ...
        $stmt = $this->conn->prepare("
            INSERT INTO empresas (
                ...
                razon_social, telefono_contacto, tipo_identificacion, cedula_ruc, latitud, longitud, sucursales, estado
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'E')
        ");
        $stmt->bind_param(
            "sssssssssssssssssss",
    ...
    public function buscarProductosYCategorias(string $q, string $categoria)
    {
        ...
        // ======================================================
        // 2) CATEGORÍAS (Relacionadas por relevancia de palabras)
        // ======================================================
        if (!empty($resp['products'])) {
            $sqlCat = "SELECT DISTINCT c.id_categoria, c.nombre, c.tipo
                   FROM categorias c
    ...
    public function createBanner($imagen)
    {
        $stmt = $this->conn->prepare("INSERT INTO banner(imagen) values(?)");
    ...
    public function updateBanner($imagen, $id)
    {
        $stmt = $this->conn->prepare("UPDATE banner SET imagen = ? WHERE id_banner = ?");
    ...
    public function createPublicidad($imagen)
    {
        $stmt = $this->conn->prepare("INSERT INTO publicidad(imagen) values(?)");
        $stmt->bind_param("s", $imagen);
        $result = $stmt->execute();
        $stmt->close();
        if ($result) {
    ...
    public function updatePublicidad($imagen, $id)
    {
        $stmt = $this->conn->prepare("UPDATE publicidad SET imagen = ? WHERE id_publicidad = ?");
        $stmt->bind_param("si", $imagen, $id);
        $result = $stmt->execute();
        $stmt->close();
        if ($result) {
    ...
    public function crearClienteApp(string $correo, string $passwordPlano, string $nombres, string $cedula, string $telefono): int
    {
        // 1. Validar si el correo ya existe en la tabla 'clientes'
        if ($this->isEmailExists($correo)) {
            return USER_ALREADY_EXISTED;
        }

        // 2. Validar si la cédula ya existe
        if ($this->isCedulaExists($cedula)) {
            return USER_CEDULA_ALREADY_EXISTED;
        }

        // 3. Si no existen, procedemos con el hash y el INSERT
        $hash = password_hash($passwordPlano, PASSWORD_DEFAULT);
        $sql = "INSERT INTO clientes (correo, password, nombres, cedula, telefono, estado)
            VALUES (?, ?, ?, ?, ?, 'A')"; // 'A' activo por defecto
        ...
        if ($stmt->execute()) {
            $stmt->close();
            // ✅ Solo si el registro es exitoso, notificamos al cliente
            $this->notificaNuevoCliente($correo, $nombres, $passwordPlano);
            return RECORD_CREATED_SUCCESSFULLY;
        } else {
            $stmt->close();
            return RECORD_CREATION_FAILED;
    ...
    private function isEmailExists($correo)
    {
    ...
    private function isCedulaExists($cedula)
    {
        $stmt = $this->conn->prepare("SELECT id_cliente from clientes WHERE cedula = ?");
    ...
    // public function validarMembresiaProductos($id_empresa, $tipo)
    // {
    ...
    //     // 2) Si el límite es 1: bloquear si ya existe un activo en productos (incluye servicios) o eventos
    //     if ($limite === 1) {
    //         $sql = "SELECT
    //                     (SELECT COUNT(*) FROM productos WHERE id_empresa=? AND estado='A') AS prod_serv,
    //                     (SELECT COUNT(*) FROM eventos   WHERE id_empresa=? AND estado='A') AS eventos";
    //         $stmt = $this->conn->prepare($sql);
    ...
    //         $ocupados = (int)$r['prod_serv'] + (int)$r['eventos'];
    //         if ($ocupados >= 1) {
    //             return [
    //                 'error' => true,
    //                 'msg'   => "Tu plan permite 1 activo total entre productos/servicios/eventos. "
    //                     . "Ya tienes: Productos/Servicios {$r['prod_serv']}, Eventos {$r['eventos']}."
    //             ];
    //         }

    //         return ['error' => false, 'msg' => 'Membresía válida (1 cupo disponible)'];
    //     }

    //     // 3) Para límites > 1: validar contra productos (que incluye servicios), como en tu lógica original
    //     $stmt = $this->conn->prepare("
    //         SELECT COUNT(*) AS total 
    //         FROM productos 
    //         WHERE id_empresa = ? AND estado = 'A'
    ...
    //     $productos_actuales = (int)$row['total'];

    //     if ($productos_actuales >= $limite) {
    //         return ['error' => true, 'msg' => "Has alcanzado el límite de $limite productos/servicios permitidos."];
    //     }
    ...
    public function validarMembresiaProductos($id_empresa, $tipo)
    {
        ...
        if ($tipo == "empresa") {
            $stmt = $this->conn->prepare("
            SELECT me.limite_articulos, me.fecha_fin, me.id_empresa
            FROM membresias_empresas me
            INNER JOIN membresias m ON m.id_membresia = me.id_membresia
            WHERE me.id_empresa = ? 
            AND me.estado = 'A' 
        ...
        $stmt->bind_param("i", $id_empresa);
        $stmt->execute();
        $resultMembresia = $stmt->get_result()->fetch_assoc();
        $stmt->close();

        if (!$resultMembresia) {
            return ['error' => true, 'msg' => 'No tiene membresía activa o su membresía ha expirado.'];
        }

        // Usamos el ID de la empresa real (por si el $id_empresa enviado era de una sucursal)
        $real_id_empresa = $resultMembresia['id_empresa'];

        $limiteRaw = trim((string)$resultMembresia['limite_articulos']);
        if (strcasecmp($limiteRaw, 'ilimitado') === 0) {
            return ['error' => false, 'msg' => 'Membresía válida (ilimitada)'];
        }
        ...
        // 2) Si el límite es 1: Bloqueo total si existe ALGO en productos, vehículos o eventos
        if ($limite === 1) {
            $sql = "SELECT
                    (SELECT COUNT(*) FROM productos WHERE id_empresa=? AND estado='A') AS prods,
                    (SELECT COUNT(*) FROM vehiculos WHERE id_empresa=? AND estado='A') AS vehs,
                    (SELECT COUNT(*) FROM eventos   WHERE id_empresa=? AND estado='A') AS evts";

            $stmt = $this->conn->prepare($sql);
            $stmt->bind_param("iii", $real_id_empresa, $real_id_empresa, $real_id_empresa);
            $stmt->execute();
            $r = $stmt->get_result()->fetch_assoc();
            $stmt->close();

            $total_existente = (int)$r['prods'] + (int)$r['vehs'] + (int)$r['evts'];

            if ($total_existente >= 1) {
                $detalle = [];
                if ($r['prods'] > 0) $detalle[] = "{$r['prods']} Producto(s)/Servicio(s)";
                if ($r['vehs'] > 0)  $detalle[] = "{$r['vehs']} Vehículo(s)";
                if ($r['evts'] > 0)  $detalle[] = "{$r['evts']} Evento(s)";

                return [
                    'error' => true,
                    'msg'   => "Tu plan actual solo permite tener 1 publicación activa en total. "
                        . "Ya tienes registrado: " . implode(", ", $detalle) . "."
                ];
            }

            return ['error' => false, 'msg' => 'Membresía válida (cupo único disponible)'];
        }

        // 3) Para límites > 1: Validar contra la suma de productos y vehículos
        // (Ajusta esta parte si el límite mayor a 1 solo debe contar productos)
        $stmt = $this->conn->prepare("
        SELECT 
            (SELECT COUNT(*) FROM productos WHERE id_empresa = ? AND estado = 'A') +
            (SELECT COUNT(*) FROM vehiculos WHERE id_empresa = ? AND estado = 'A') AS total
    ");
        $stmt->bind_param("ii", $real_id_empresa, $real_id_empresa);
        $stmt->execute();
        $row = $stmt->get_result()->fetch_assoc();
        $stmt->close();
        ...
        if ($ocupados >= $limite) {
            return [
                'error' => true,
                'msg' => "Has alcanzado el límite de $limite publicaciones permitidas en tu plan actual."
            ];
        }

        return ['error' => false, 'msg' => 'Membresía válida y cupo disponible'];
    }
    ...
    public function upgradeMembresia($id_empresa, $id_membresia, $valor, $sucursales)
    {
        ...
        try {
            $this->validarUpgradeEstricto($actual, $nueva, $valor);
        } catch (\Throwable $e) {
        ...
        */
        ...
        $stmt = $this->conn->prepare("UPDATE membresias_empresas SET estado='E', fecha_fin=? WHERE id_membresia_empresa=?");
        $stmt->bind_param("si", $finAhora, $actual['id_membresia_empresa']);
        $stmt->execute();
        $stmt->close();
    ...
    // public function createVehiculo($id_modelo, $anio, $condicion, $tipo_auto, $id_marca, $kilometraje, $transmision, $tipo_traccion, $funcionamiento_motor, $inicio_placa, $fin_placa, $tipo_vendedor, $provincia, $canton, $img_frontal, $img_posterior, $color, $cilindraje, $tapiceria, $duenio, $direccion, $climatizacion, $descripcion, $precio_referencia, $id_empresa, $descuento, $tipo_creador, $negociable, $estado, $tags, $referencias, $iva)
    // {
    //     // Normalizar JSONs
    //     $condicionJson     = json_encode((array)$condicion);
    //     $transmisionJson   = json_encode((array)$transmision);
    //     $tipoVendedorJson  = json_encode((array)$tipo_vendedor);
    //     $provinciaJson     = json_encode((array)$provincia);
    //     $cantonJson        = json_encode((array)$canton);
    //     $tapiceriaJson     = json_encode((array)$tapiceria);
    //     $duenioJson        = json_encode((array)$duenio);
    //     $direccionJson     = json_encode((array)$direccion);
    //     $climatizacionJson = json_encode((array)$climatizacion);
    //     $referenciasJson   = json_encode((array)$referencias);

    //     $query = "INSERT INTO vehiculos (
    //             id_modelo, anio, condicion, tipo_auto, id_marca, kilometraje, 
    //             transmision, tipo_traccion, funcionamiento_motor, inicio_placa, 
    //             fin_placa, tipo_vendedor, provincia, canton, img_frontal, 
    //             img_posterior, color, cilindraje, tapiceria, tipo_dueno, 
    //             direccion, climatizacion, descripcion, precio_referencia, 
    //             id_empresa, descuento, tipo_creador, negociable, estado, tags, referencias, iva
    //         ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

    //     $stmt = $this->conn->prepare($query);
    //     if (!$stmt) {
    //         return ["error" => true, "msg" => "Prepare failed: " . $this->conn->error];
    //     }

    //     // CADENA DE TIPOS VERIFICADA: 24 's', 1 'i', 1 'd', 1 's', 1 'i', 3 's', 1 'i'
    //     $types = "ssssssssssssssssssssssssisdisssi";
    ...
    //         $stmt->close();
    //         return ["error" => true, "msg" => "Execute failed: " . $error_msg];
    //     }
    ...
    //     // Guardar archivos
    //     if (!empty($archivos)) {
    //         $lista = isset($archivos['archivos']) ? $archivos['archivos'] : (is_array($archivos) ? $archivos : []);
    //         if (!empty($lista)) {
    //             $stmtArc = $this->conn->prepare("INSERT INTO archivos_vehiculos (id_vehiculo, archivo, tipo) VALUES (?, ?, ?)");
    //             foreach ($lista as $arc) {
    //                 $path = is_array($arc) ? ($arc['archivo'] ?? '') : $arc;
    //                 $tipo = is_array($arc) ? ($arc['tipo'] ?? 'imagen') : 'imagen';
    //                 if (!empty($path)) {
    //                     $stmtArc->bind_param("iss", $ultimo_id, $path, $tipo);
    //                     $stmtArc->execute();
    //                 }
    //             }
    //             $stmtArc->close();
    ...
    //     $cantonJson        = json_encode((array)$canton);
    //     $tapiceriaJson     = json_encode((array)$tapiceria);
    //     $duenioJson        = json_encode((array)$tipo_dueno);
    //     $direccionJson     = json_encode((array)$direccion);
    //     $climatizacionJson = json_encode((array)$climatizacion);
    //     $referenciasJson   = json_encode((array)$referencias);
    ...
    //     // 32 parámetros: s=28, i=3 (id_empresa, negociable, iva), d=1 (descuento)
    //     // El orden de los tipos debe ser: ssss ss ss s ssss ss ss ss ss ss s i d s i s s s i
    //     // Simplificado a cadena de tipos:
    //     $types = "ssssssssssssssssssssssssisdisssi";
    ...
    //     if ($result) {
    //         // ... (Tu lógica de guardado de archivos se mantiene igual)
    //         return RECORD_CREATED_SUCCESSFULLY;
    //     }
    //     return RECORD_CREATION_FAILED;
    // }
    ...
    */
    ...
    public function deleteArchivoProducto($id_archivo_producto)
    {
        // Preparamos el query para cambiar el estado a 'E' (Eliminado)
        $stmt = $this->conn->prepare("UPDATE archivos_productos SET estado = 'E' WHERE id_archivo_producto = ?");

        // "i" si el ID es entero, "s" si es string (asumo entero por ser un ID)
        $stmt->bind_param("i", $id_archivo_producto);
    ...
    public function eliminarCuentaCliente($id_usuario)
    {
        // Cambiamos el estado a 'E' (Eliminado) en lugar de borrar el registro
        $stmt = $this->conn->prepare("UPDATE clientes SET estado = 'E' WHERE id_usuario = ?");
        $stmt->bind_param("i", $id_usuario);
        if ($stmt->execute()) {
            return RECORD_CREATED_SUCCESSFULLY;
        }
