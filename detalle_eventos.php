...
<div class="container event-shell">
    <div class="shop-product-fillter mb-4">
    ...
    <div class="event-hero-card">
        ...
        <div class="event-hero-body">
            ...
            <div id="empresa_redes" class="event-social-links" style="display:none;"></div>
        </div>
    </div>
    ...
    <div class="event-grid">
        <div>
            ...
            <div class="event-panel mb-4" id="box-empresa-descripcion" style="display:none;">
                <div class="event-panel-title">Sobre la empresa</div>
                <div class="event-company-description" id="descripcion_empresa_evento"></div>
            </div>
            ...
            <div class="event-panel" id="box-ubicacion" style="display:none;">
                ...
                <div class="mt-3" id="wrap-coordenadas" style="display:none;">
                    <h6 class="mb-1">Coordenadas</h6>
                    <div class="text-muted" style="line-height:1.6;">
                        <span id="txt-latitud"></span>
                        <span id="txt-longitud"></span>
                    </div>
                </div>
...
<script>
    ...
    function titleCaseWords(value, fallback = "No especificado") {
        const txt = (value ?? "").toString().trim();
    ...
    function renderSubtipos(subtipos) {
        ...
        wrap.innerHTML = list.map(item => {
            ...
            return `<span class="event-outline-badge">${titleCaseWords(nombre, "")}</span>`;
        }).join("");
    }

    function renderRedesEmpresa(ev) {
        const wrap = document.getElementById("empresa_redes");
        if (!wrap) return;
        ...
        const links = redes
            .map(red => {
                const href = safeUrl(ev?.[red.key]);
                if (!href) return "";
                return `
                    <a class="event-social-link" href="${href}" target="_blank" rel="noopener" aria-label="${red.label}" title="${red.label}">
                        <i class="${red.icon}"></i>
                    </a>
                `;
            })
    ...
    function renderDescripcionEmpresa(ev) {
        const box = document.getElementById("box-empresa-descripcion");
        const desc = (ev?.empresa_descripcion || "").toString().trim();
        if (!box) return;

        if (!desc) {
            box.style.display = "none";
            $("#descripcion_empresa_evento").text("");
            return;
        }

        box.style.display = "block";
        $("#descripcion_empresa_evento").text(desc);
    }
    ...
    function renderAcciones(ev) {
        ...
        if (enlace) {
            actions.push(`
                <a class="btn btn-primary w-100" href="${enlace}" target="_blank" rel="noopener">
        ...
        if (compra) {
            actions.push(`
                <a class="btn btn-outline-primary w-100" href="${compra}" target="_blank" rel="noopener">
                    Enlace compra
                </a>
            `);
        }
    ...
    $.post(`api/v1/fulmuv/eventos/ById`, {
            ...
            // Título
            $("#titulo_evento").text(titleCaseWords(ev.titulo || "Evento", "Evento"));
            renderGaleria(ev.galeria);
            renderContacto(ev);
            renderUbicacion(ev);
            renderRedesEmpresa(ev);
            renderDescripcionEmpresa(ev);

            // Empresa con link
            const nombreOrg = titleCaseWords(ev.organizador || ev.empresa_nombre || ev.nombre_contacto || "Organizador", "Organizador");
            ...
            $("#hero_tipo").text(titleCaseWords(ev.tipo || ev.tipo_evento, "Evento"));

            const modalidad = titleCaseWords(ev.modalidad, "");
            if (modalidad) {
            ...
            const tipoEntrada = titleCaseWords(ev.tipo_entrada, "");
            if (tipoEntrada) {
            ...
            $("#modalidad_evento").text(titleCaseWords(ev.modalidad));
            $("#tipo_entrada_evento").text(titleCaseWords(ev.tipo_entrada));
    ...
    function renderContacto(ev) {
        ...
        const nombre = (ev.nombre_contacto || ev.contacto || ev.empresa_nombre || ev.organizador || "").toString().trim();
        ...
        // nombre
        $("#contacto-nombre").text(nombre ? titleCaseWords(nombre, "—") : "—");
        ...
        <div class="d-flex align-items-center gap-2">
          ...
          <a class="btn btn-sm btn-success" target="_blank" rel="noopener" href="${waHref}">
            WhatsApp
          </a>
          <a class="btn btn-sm btn-outline-primary" href="${telHref}">
            Llamar
          </a>
        </div>
      `);
    ...
    function registrarInteraccionEventoMapa() {
        ...
        if (!session) {
            session = `track_${Date.now()}_${Math.random().toString(36).slice(2, 12)}`;
        ...
        $.ajax({
            ...
            dataType: "json",
            data: {
                id_producto: idEvento,
                tipo_evento: "click_mapa",
                cantidad: 1,
                session_key: session,
                detalle_url: window.location.href,
                referencia: document.referrer || "",
                metadata: JSON.stringify({ origen: "detalle_evento" }),
                tipo: "evento"
            }
        });
    }
    ...
    function renderUbicacion(ev) {
        ...
        if (prov || cant) {
            ...
            $("#txt-provincia").text(prov ? `Provincia: ${titleCaseWords(prov, "")}` : "");
            $("#txt-canton").text(cant ? `${prov ? " · " : ""}Cantón: ${titleCaseWords(cant, "")}` : "");
        } else {
        ...
        if (hasCoords) {
            $("#wrap-coordenadas").show();
            $("#txt-latitud").text(`Latitud: ${lat}`);
            $("#txt-longitud").text(` · Longitud: ${lng}`);
        } else {
            $("#wrap-coordenadas").hide();
            $("#txt-latitud").text("");
            $("#txt-longitud").text("");
        }
            ...
            waitMapsReady(() => {
                ...
                if (!__mapEvento) {
                    ...
                    __markerEvento.addListener("click", () => {
                        registrarInteraccionEventoMapa();
